...
Study-wide modeled syntheses follow `study-modeling-policy-1.3.0`. Already-public observations are searched first, source projections remain separate, and a modeled value is eligible only when the metric is decision-relevant, inputs are cited, the formula is reproducible, scope is defensible, and uncertainty is disclosed. Otherwise the unresolved evidence gap remains documented as an analytical limit. The study sends no public-records requests, private data requests, surveys, questionnaires, or other solicitations. Benefits and costs face the same evidence rules. Fiscal components can be netted only when government level, recipient scope, accounting basis, and time horizon align. Public-service cost is not inferred as an arbitrary share of revenue: when same-scope marginal cost is unavailable, the account reports recurring tax contribution and a cost break-even threshold without asserting a net fiscal result. Construction multipliers apply only to construction-eligible spending. Mechanically matched county comparisons are descriptive gaps and are never labeled as facility effects.
...
Release 1.43 applies a machine-enforced gate for a completed contribution account. The gate requires coverage of investment, construction, suppliers, operations, fiscal revenue, public costs, resources and community contributions, plus separate descriptive county GDP, employment and wage comparison modules. A field may be populated by a direct source record or an explicitly labeled synthesis. Passing the gate means the contribution account is populated; it does not mean all values were observed, audited or causally identified. Source details, unresolved gaps, assumptions and limitations remain available in the audit ledger.
