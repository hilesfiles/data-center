...
5 September 2026. Companion to the [revised study plan](revised-private-sector-economic-study-plan.md). The application remediation is implemented locally through `private-sector-study-1.38.0`, including browser verification. Release 2 now contains 584 source records across three depth accounts and the first separately governed modeled-synthesis layer. Complete annual accounts and Release 3 remain the implementation roadmap. The public deployment has not been changed.
...
Study release `private-sector-study-1.38.0` publishes 584 source-checked records across all 36 candidates—532 reported observations and 52 forecasts—plus eight separately governed Apple Mesa syntheses. The three depth accounts cover Meta Forest City, Digital Crossroad Hammond and Switch Citadel. Storey retains its real- and personal-property accounts, tax bills and verified payments, Fire District contributions and paired equipment-cost forecast. Apple Mesa adds operating electricity, renewable capacity, permit valuations, cumulative investment, employment and incentive forecasts, FTZ context, and transparently modeled water, employment, payroll and incentive counterfactuals. The generic synthesis contract and machine-readable policy apply across facilities and counties without publishing estimates for another real facility. Typed intervals, contribution channels, aggregation controls, named parameter provenance and causal or multiplier requirements keep models outside canonical claims and realized-benefit totals.
...
| Area | Current implementation | Remediation |
|---|---|---|
| Navigation and profiles | `site/src/App.tsx` recognizes county hash routes, loads county-level products, and presents first-entry status in both the map sidebar and county profile. | Add a searchable study-project register and stable project routes; connect project profiles to existing county routes and map selections. Add campus/facility detail only where it supports project identity and history. |
| Main status messaging | The banner and caption in `App.tsx` hard-code first-entry counts, tranche narratives, dates, and queue labels. | Show generated study coverage and evidence availability in the primary view. Retain historical research detail in a methodology/evidence view. Generate counts and vintages from the same release as the displayed records. |
| Map and county context | `MapPanel.tsx` supports source coverage and county economic measures. The default is IM3 source-record coverage. | Retain these layers as context. Add selection/filtering for study projects, using verified coordinates or labeled campus locations. Show distinct counts for source objects, campuses, canonical facilities, study projects, and verified operating facilities. |
| Domain model | Project, phase, event, claim, observation, and operator-relationship schemas already exist. | Reuse those entities and IDs; materialize reviewed project records and their links. Add a study-membership/readiness contract, ownership/purpose classification where needed, and explicit evidence coverage. Do not create a second inventory disconnected from the existing one. |
...
| Model configuration | The only registered model is employment around first operational county entry. Treatment definitions include first entry and construction; the treatment schema also permits operation and expansion. | Add separately versioned project opening/expansion definitions and outcome-specific model specifications. Review construction eligibility for reuse. Keep legacy first-entry rules and results intact under their existing identity. |
...
| Publication | Python builds JSON; the React application consumes static files. GitHub Actions validates and builds before Pages deployment. | Extend the same pipeline with study-register, project-detail, evidence, and annual-account projections. Use manifests and lazy loading; preserve the existing county URLs and public contracts until consumers migrate. |
...
- Add project search/filtering, project routes, and links from county profiles and map selections.
...
**Acceptance:** All 36 starting candidates can be found through the application and traced to existing entities and evidence. Campus/building differences and date bounds survive publication. Existing county links work. A first-entry rejection does not hide a candidate project. The interface distinguishes research candidacy, operating-status verification, and analysis readiness.

### Release 2: Annual economic and fiscal accounts

- Add acquisition/review workflows and governed observations for construction, capital spending, operations, suppliers, tax base, receipts, incentives, and public costs.
...
- Add annual benefit and fiscal displays with source drill-down and method labels for supplier and household-spending estimates.
...
- Add cooling, water use, energy, and infrastructure-cost evidence as it becomes available; display context layers within permitted data-use terms.

**Acceptance:** Every displayed amount has a defined metric, period, subject, source or derivation, and evidence status. Campus/phase aggregation avoids duplication. Job-years and peak jobs differ. Announced spending is not realized spending. Missing or suppressed values are not zeros. Actual tax receipts already net of abatements are not reduced by the same abatements again. Available cases can publish before all 36 have complete records.
...
- Add model runs, diagnostics, uncertainty, and public projections through the existing analytical record framework.
...
**Acceptance:** First-entry rules continue to mean first entry; the new analysis does not bypass them. Each new estimate has its own versioned definition, run, diagnostics, and eligible sample. A project may have usable fiscal evidence while remaining ineligible for a causal employment estimate.
...
- Generate a coherent release manifest with counts, vintages, and publication paths. Avoid coupling the UI to one named research tranche as the permanent source of current status.
