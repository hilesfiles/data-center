...
5 September 2026. Companion to the [revised study plan](revised-private-sector-economic-study-plan.md). The application remediation is implemented locally through `private-sector-study-1.43.0`, including browser verification. The release contains 585 source records and 104 separately governed modeled syntheses. Apple Mesa / Maricopa County, Switch Citadel / Storey County and Digital Crossroad Hammond / Lake County pass the machine-enforced completed-contribution-account gate. The fiscal account reports recurring local tax contributions and public-cost break-even thresholds without claiming an unsupported net result; county comparison gaps are explicitly noncausal.
...
Study release `private-sector-study-1.43.0` publishes 585 source-checked records across all 36 candidates—533 reported observations and 52 forecasts—plus 104 separately governed syntheses: 27 for Apple Mesa, 34 for Switch Citadel / Tahoe Reno 1 and 43 for Digital Crossroad Hammond. The three accounts cover bounded construction and operating contribution channels, fiscal evidence and break-even thresholds, resource engineering and explicitly noncausal county comparisons. Typed intervals, contribution channels, aggregation controls, named parameter provenance and causal or multiplier requirements keep models outside canonical claims and realized-benefit totals.
...
| Map and county context | `MapPanel.tsx` renders neutral Census county geography, highlights the three full modeled county accounts and links their project markers to profiles. | Preserve the legacy IM3 facility projection and county datasets off-map for future study. Add a layer back only when its research purpose and evidence status are explicit. |
