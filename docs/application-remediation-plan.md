...
7 September 2026. Companion to the [revised study plan](revised-private-sector-economic-study-plan.md). The application remediation is implemented through `private-sector-study-1.55.0`, including browser verification. The release contains 1,046 source records and 116 separately governed modeled syntheses. The latest audited batch adds Stream Houston I / The Woodlands, TierPoint Charlotte CL4, and TierPoint / Windstream Little Rock, while preserving the foundational re-audit's removal of premature models.
...
Study release `private-sector-study-1.55.0` publishes 1,046 source-checked records across all 36 candidates—940 reported observations and 106 forecasts—plus 116 separately governed syntheses. Twenty-four projects have completed scoped research audits and are map-eligible; no project currently passes the separate full modeled-account gate. Renewed direct-source discovery added permit, tax, assessment, facility, employment, power, and community evidence while model-by-model review retained only decision-useful arithmetic or bounded counterfactuals.
