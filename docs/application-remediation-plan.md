...
Study release `private-sector-study-1.58.0` publishes 1,487 source-checked records across all 36 candidates—1,354 reported observations and 133 forecasts—plus 221 separately governed syntheses. Thirty-three projects have completed scoped research audits and are map-eligible; no project currently passes the separate full modeled-account gate. Renewed direct-source discovery added permit, tax, assessment, facility, employment, power, water, public-infrastructure, supplier, incentive, and community evidence while model-by-model review retained only decision-useful arithmetic, governed source-model outputs, or bounded counterfactuals.
