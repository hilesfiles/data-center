...
6 September 2026. Companion to the [revised study plan](revised-private-sector-economic-study-plan.md). The application remediation is implemented through `private-sector-study-1.47.0`, including browser verification. The release contains 660 source records and 138 separately governed modeled syntheses. Three projects pass the machine-enforced completed-contribution-account gate: Apple Mesa / Maricopa County, Switch Citadel / Storey County, and Digital Crossroad Hammond / Lake County. Corrective audits of Meta Forest City, Microsoft San Antonio, and EdgeConneX DET01 added thirteen direct or source-forecast records and removed 38 premature models; all three now remain explicitly incomplete where supplier, construction, operating, resource, or public-cost evidence cannot be defensibly bounded. Fiscal presentation reports recurring local tax contributions and public-cost break-even thresholds only where same-scope evidence supports them, without claiming an unsupported net result; county comparison gaps are explicitly noncausal.

## Release 1 implementation status

The default application opens a searchable 36-project register covering 35 counties and 23 states. It includes all five campus candidates, project profiles with stored historical observations and evidence links, eight economic-evidence categories, separate analysis-readiness indicators, and links to existing county histories. The map shows clickable markers and highlighted counties for all nine projects whose scoped audits are complete: the original three accounts plus the six completed Batch 1 and Batch 2 audits. The independent analytical gate still identifies three full modeled accounts. The legacy national facility inventory, county datasets and the complete 36-project register remain stored and accessible off-map. First-entry adjudications are retained in expandable research sections rather than primary study eligibility cards.
...
Study release `private-sector-study-1.47.0` publishes 660 source-checked records across all 36 candidates—597 reported observations and 63 forecasts—plus 138 separately governed syntheses across three completed accounts. Nine projects have completed scoped research audits and are map-eligible; the separate full modeled-account gate remains at three. The corrective Batch 1 audit added twelve reported observations and one forecast while removing 38 models that could not survive the auditable-search-exhaustion standard. Meta Forest City retains six bounded syntheses, Microsoft San Antonio retains three descriptive county comparisons, and EdgeConneX DET01 retains three fiscal/resource syntheses; all three accounts remain incomplete. Typed intervals, contribution channels, aggregation controls, named parameter provenance, and causal or multiplier requirements keep models outside canonical claims and realized-benefit totals.
...
| Map and county context | `MapPanel.tsx` renders neutral Census county geography, highlights the six full modeled county accounts and links their project markers to profiles. | Preserve the legacy IM3 facility projection and county datasets off-map for future study. Add a layer back only when its research purpose and evidence status are explicit. |
