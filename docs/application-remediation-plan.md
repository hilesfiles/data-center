...
6 September 2026. Companion to the [revised study plan](revised-private-sector-economic-study-plan.md). The application remediation is implemented through `private-sector-study-1.53.0`, including browser verification. The release contains 947 source records and 161 separately governed modeled syntheses. Three projects pass the machine-enforced completed-contribution-account gate: Apple Mesa / Maricopa County, Switch Citadel / Storey County, and Digital Crossroad Hammond / Lake County. Corrective model-audit Batch B revisits Flexential Chaska, Markley Lowell, and Meta Los Lunas under the direct-evidence-first contract. It adds nine reported observations and one eligible last-resort synthesis; all three remain explicitly incomplete where public evidence cannot defensibly bound material gaps.

## Release 1 implementation status

The default application opens a searchable 36-project register covering 35 counties and 23 states. It includes all five campus candidates, project profiles with stored historical observations and evidence links, eight economic-evidence categories, separate analysis-readiness indicators, and links to existing county histories. The map shows clickable markers and highlighted counties for all twenty-one projects whose scoped audits are complete: the original three accounts plus the eighteen completed Batch 1 through Batch 6 audits. The independent analytical gate still identifies three full modeled accounts. The legacy national facility inventory, county datasets and the complete 36-project register remain stored and accessible off-map. First-entry adjudications are retained in expandable research sections rather than primary study eligibility cards.
...
Study release `private-sector-study-1.53.0` publishes 947 source-checked records across all 36 candidates—842 reported observations and 105 forecasts—plus 161 separately governed syntheses. Twenty-one projects have completed scoped research audits and are map-eligible; the separate full modeled-account gate remains at three. Corrective model-audit Batch B adds nine reported observations and one eligible synthesis after renewed direct-source discovery and candidate-by-candidate modeling review. Chaska adds five exact parcel-value stocks and rejects every model candidate; Lowell adds one recipient-verified contribution and a same-account public-cost break-even; Los Lunas adds three campus-level operations and community observations and rejects every model candidate. Typed intervals, contribution channels, aggregation controls, named parameter provenance, and causal or multiplier requirements keep models outside canonical claims and realized-benefit totals.
