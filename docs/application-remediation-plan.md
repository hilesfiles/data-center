...
Study release `private-sector-study-1.57.0` publishes 1,230 source-checked records across all 36 candidates—1,112 reported observations and 118 forecasts—plus 146 separately governed syntheses. Thirty projects have completed scoped research audits and are map-eligible; no project currently passes the separate full modeled-account gate. Renewed direct-source discovery added permit, tax, assessment, facility, employment, power, resource, incentive, and community evidence while model-by-model review retained only decision-useful arithmetic or bounded counterfactuals.
