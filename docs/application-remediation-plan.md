...
6 September 2026. Companion to the [revised study plan](revised-private-sector-economic-study-plan.md). The application remediation is implemented through `private-sector-study-1.54.0`, including browser verification. The release contains 995 source records and 66 separately governed modeled syntheses. The foundational re-audit adds 48 net source records and removes 95 premature models from Apple Mesa, Switch Citadel, and Digital Crossroad Hammond; all three are now correctly classified as analytically incomplete where public evidence cannot defensibly bound material gaps.
...
Study release `private-sector-study-1.54.0` publishes 995 source-checked records across all 36 candidates—889 reported observations and 106 forecasts—plus 66 separately governed syntheses. Twenty-one projects have completed scoped research audits and are map-eligible; no project currently passes the separate full modeled-account gate. Renewed direct-source discovery added permit, tax, assessment, environmental, utility-regulatory, employment, capital, and community evidence while model-by-model review retained only decision-useful arithmetic or bounded counterfactuals.
