...
6 September 2026. Companion to the [revised study plan](revised-private-sector-economic-study-plan.md). The application remediation is implemented through `private-sector-study-1.50.0`, including browser verification. The release contains 769 source records and 149 separately governed modeled syntheses. Three projects pass the machine-enforced completed-contribution-account gate: Apple Mesa / Maricopa County, Switch Citadel / Storey County, and Digital Crossroad Hammond / Lake County. Batch 5 audits of Flexential Minneapolis-Chaska, Markley Lowell, and Meta Los Lunas add a net 45 reported observations and six source forecasts after removing six Stream/IP Stream incentive rows that do not belong to the mapped Flexential facility. No modeled syntheses were added; all three remain explicitly incomplete where public evidence cannot defensibly bound material gaps.

## Release 1 implementation status

The default application opens a searchable 36-project register covering 35 counties and 23 states. It includes all five campus candidates, project profiles with stored historical observations and evidence links, eight economic-evidence categories, separate analysis-readiness indicators, and links to existing county histories. The map shows clickable markers and highlighted counties for all eighteen projects whose scoped audits are complete: the original three accounts plus the fifteen completed Batch 1 through Batch 5 audits. The independent analytical gate still identifies three full modeled accounts. The legacy national facility inventory, county datasets and the complete 36-project register remain stored and accessible off-map. First-entry adjudications are retained in expandable research sections rather than primary study eligibility cards.
...
Study release `private-sector-study-1.50.0` publishes 769 source-checked records across all 36 candidates—693 reported observations and 76 forecasts—plus 149 separately governed syntheses. Eighteen projects have completed scoped research audits and are map-eligible; the separate full modeled-account gate remains at three. Batch 5 adds 51 net source records and no modeled syntheses after persistent project-specific source review. Typed intervals, contribution channels, aggregation controls, named parameter provenance, and causal or multiplier requirements keep models outside canonical claims and realized-benefit totals.
