...
Study release `private-sector-study-1.56.0` publishes 1,091 source-checked records across all 36 candidates—980 reported observations and 111 forecasts—plus 125 separately governed syntheses. Twenty-seven projects have completed scoped research audits and are map-eligible; no project currently passes the separate full modeled-account gate. Renewed direct-source discovery added permit, tax, assessment, facility, employment, power, resource, incentive, and community evidence while model-by-model review retained only decision-useful arithmetic or bounded counterfactuals.
