...
8 September 2026. Companion to the [revised study plan](revised-private-sector-economic-study-plan.md). The application remediation is implemented through `private-sector-study-1.59.0`, including browser verification. The release contains 1,700 source records and 278 separately governed modeled syntheses. The final audited batch adds Google Berkeley County, Google Bridgeport/Jackson County, and Google Lenoir, promotes source-reported environmental and design values into the direct catalog, and removes deterministic transcriptions from modeled synthesis.

## Release 1 implementation status

The default application opens a searchable 36-project register covering 35 counties and 23 states. It includes all five campus candidates, project profiles with stored historical observations and evidence links, eight economic-evidence categories, separate analysis-readiness indicators, and links to existing county histories. The map shows clickable markers and highlighted counties for all 36 projects whose scoped audits are complete. Google Berkeley County passes the independent mechanical full modeled-account gate; the remaining 35 accounts retain explicit gaps. The legacy national facility inventory, county datasets and the complete 36-project register remain stored and accessible off-map. First-entry adjudications are retained in expandable research sections rather than primary study eligibility cards.
...
Study release `private-sector-study-1.59.0` publishes 1,700 source-checked records across all 36 candidates—1,539 reported observations and 161 forecasts—plus 278 separately governed syntheses. All 36 projects have completed scoped research audits and are map-eligible; Google Berkeley County passes the separate mechanical full modeled-account gate. Renewed direct-source discovery added permit, tax, assessment, facility, employment, power, water, emissions, public-infrastructure, supplier, incentive, and community evidence while model-by-model review retained only decision-useful arithmetic, governed source-model outputs, or bounded counterfactuals.
