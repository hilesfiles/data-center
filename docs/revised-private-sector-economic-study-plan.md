...
5 September 2026. Revised research and implementation sequence. Release `private-sector-study-1.38.0` combines 584 economic source records across all 36 candidates—532 reported observations and 52 projections—with eight separately governed Apple Mesa syntheses. It preserves the Storey County depth account and generalizes the modeled-synthesis contract for future facility and county work. The versioned modeling policy requires direct-evidence search first, separate forecasts, reproducible parameters, explicit interval kinds, defensible scope, contribution-channel separation, anti-overlap aggregation and method-specific causal or multiplier metadata. Complete annual accounts and project-event impact analysis remain subsequent work.
