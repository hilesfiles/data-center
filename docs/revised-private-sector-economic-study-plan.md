...
6 September 2026. Revised research and implementation sequence. Release `private-sector-study-1.47.0` combines 660 economic source records across all 36 candidates—597 reported observations and 63 projections—with 138 separately governed syntheses across three completed contribution accounts. The selected register spans 35 counties and 23 states. Nine projects have completed scoped research audits and are mapped; three currently pass the separate completed-contribution-account analytical gate, and 33 remain analytically incomplete. The versioned modeling policy requires auditable public-source exhaustion before last-resort synthesis, separate forecasts, reproducible parameters, explicit interval kinds, defensible scope, contribution-channel separation, anti-overlap aggregation and method-specific causal or multiplier metadata. The current release publishes no pooled or causal estimate and makes no net-fiscal claim without same-scope public-cost evidence.
...
The 36-project register is accessible through the application, and all candidates have at least one quantitative source record. The economic-evidence register contains 660 source-checked records: 597 reported records and 63 forecasts. The 138 modeled syntheses are counted separately; three accounts currently pass the full gate and 33 remain incomplete. Batch 2 demonstrates that a persistent source search can add 52 direct/forecast records while removing weak models. The corrective Batch 1 audit then added twelve observations and one forecast while removing 38 models that were not defensible after wider assessor, tax, permit, utility, operator, community, incentive, procurement, and public-cost searches. No current row is a pooled estimate or causal effect. Continue with the remaining project accounts, then build the pooled artifacts and run the registered model ladder without collapsing evidence states or treating multiple projects in one county as independent outcomes.
