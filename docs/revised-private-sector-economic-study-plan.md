...
6 September 2026. Revised research and implementation sequence. Release `private-sector-study-1.53.0` combines 947 economic source records across all 36 candidates—842 reported observations and 105 projections—with 161 separately governed syntheses. The selected register spans 35 counties and 23 states. Twenty-one projects have completed scoped research audits and are mapped; three currently pass the separate completed-contribution-account analytical gate, and 33 remain analytically incomplete. The versioned modeling policy requires auditable public-source exhaustion before last-resort synthesis, separate forecasts, reproducible parameters, explicit interval kinds, defensible scope, contribution-channel separation, anti-overlap aggregation and method-specific causal or multiplier metadata. The current release publishes no pooled or causal estimate and makes no net-fiscal claim without same-scope public-cost evidence.
...
The 36-project register is accessible through the application, and all candidates have at least one quantitative source record. The economic-evidence register contains 947 source-checked records: 842 reported records and 105 forecasts. The 161 modeled syntheses are counted separately; three accounts currently pass the full gate and 33 remain incomplete. Completed corrective and evidence-persistence audits demonstrate that broader assessor, tax, permit, utility, operator, community, incentive, procurement, workforce, and public-cost searches can add direct evidence while removing or avoiding weak models. Corrective model-audit Batch B added nine reported observations and retained one bounded synthesis only after candidate-specific eligibility review. No current row is a pooled estimate or causal effect. The foundational full-account re-audit of Apple Mesa, Switch Citadel, and Digital Crossroad Hammond precedes ordinary Batch 7; after that, continue remaining project accounts, then build pooled artifacts and run the registered model ladder without collapsing evidence states or treating multiple projects in one county as independent outcomes.
