...
6 September 2026. Revised research and implementation sequence. Release `private-sector-study-1.45.0` combines 647 economic source records across all 36 candidates—585 reported observations and 62 projections—with 176 separately governed syntheses across six completed contribution accounts. The selected register spans 35 counties and 23 states. Six projects currently pass the completed-contribution-account gate; 30 remain incomplete. The versioned modeling policy requires auditable public-source exhaustion before last-resort synthesis, separate forecasts, reproducible parameters, explicit interval kinds, defensible scope, contribution-channel separation, anti-overlap aggregation and method-specific causal or multiplier metadata. The current release publishes no pooled or causal estimate and makes no net-fiscal claim without same-scope public-cost evidence.
...
The 36-project register is accessible through the application, and all candidates have at least one quantitative source record. The economic-evidence register contains 647 source-checked records: 585 reported records and 62 forecasts. The 176 modeled syntheses are counted separately; six accounts currently pass the full gate and 30 remain incomplete. Batch 2 demonstrates that a persistent source search can add 52 direct/forecast records while removing weak models: the three accounts retain 22 bounded residual syntheses but remain open where supplier or community evidence cannot be bounded. No current row is a pooled estimate or causal effect. Continue with the remaining project accounts, then build the pooled artifacts and run the registered model ladder without collapsing evidence states or treating multiple projects in one county as independent outcomes.
