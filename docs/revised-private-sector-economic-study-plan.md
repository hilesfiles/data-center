...
6 September 2026. Revised research and implementation sequence. Release `private-sector-study-1.44.0` combines 595 economic source records across all 36 candidates—539 reported observations and 56 projections—with 154 separately governed syntheses across six completed contribution accounts. The selected register spans 35 counties and 23 states. Six projects currently pass the completed-contribution-account gate; 30 remain incomplete. The versioned modeling policy requires direct-evidence search first, separate forecasts, reproducible parameters, explicit interval kinds, defensible scope, contribution-channel separation, anti-overlap aggregation and method-specific causal or multiplier metadata. The current release publishes no pooled or causal estimate and makes no net-fiscal claim without same-scope public-cost evidence.
...
The 36-project register is accessible through the application, and all candidates have at least one quantitative source record. The economic-evidence register contains 595 source-checked records: 539 reported records and 56 forecasts. The 154 modeled syntheses are counted separately across six completed contribution accounts; the other 30 projects require the same facility-year evidence and synthesis work before entering the county-year exposure panel. The completed accounts separate reported anchors, construction and operating contribution scenarios, recurring local tax contributions and public-cost break-even thresholds where supported, public support, physical resource demand, and descriptive county comparisons. No current row is a pooled estimate or causal effect. Continue with the remaining project accounts, then build the pooled artifacts and run the registered model ladder without collapsing evidence states or treating multiple projects in one county as independent outcomes.
