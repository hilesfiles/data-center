...
try {
  ...
  await page.getByText("113 sourced records · 37 modeled syntheses", { exact: true }).waitFor();
  ...
  assert.equal(await page.locator(".modeled-record").count(), 37);
  ...
  assert.match(await hammondCoverage.innerText(), /Electricity, water, and cooling[\s\S]*5 reported[\s\S]*0 forecast[\s\S]*6 modeled/);
  ...
  assert.match(await hammondReadiness.innerText(), /Local fiscal balance[\s\S]*Partial modeled account[\s\S]*59 reported · 4 forecast · 5 modeled/);
  assert.match(await hammondReadiness.innerText(), /Attributable economic effects[\s\S]*Not model-ready[\s\S]*0 causal models/);
  ...
  check("Hammond keeps sourced, forecast and 37 modeled records separate while populating coverage and readiness");
  await page.getByRole("tab", { name: /Reported activity/ }).click();
  await noOverflow();
