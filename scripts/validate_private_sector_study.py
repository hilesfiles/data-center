...
def validate_study(validator):
    ...
    try:
        ...
        expected_index, expected_details, expected_entities = build_products(config, inventory, panels, index["generated_at"])
        _, expected_claims, expected_sources = economic_products(read(EVIDENCE), config["candidates"], index["generated_at"])
        ...
        expected_parts = {"site/public/data/v1/study/index.json", "data/silver/study/projects.json", "data/silver/study/economic-claims.json", "data/silver/study/economic-sources.json", *[f"site/public/data/v1/study/{r['detail_path']}" for r in index["projects"]]}
        ...
        required_inputs = {"config/v1/study-economic-evidence.json", "scripts/study_economic_evidence.py", "config/v1/private-sector-study-candidates.json", "scripts/build_private_sector_study.py"}
