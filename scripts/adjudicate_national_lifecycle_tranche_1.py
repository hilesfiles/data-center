#!/usr/bin/env python3
"""Build the first governed national lifecycle-evidence tranche."""
...
DATASET_ID = "im3_lifecycle_national_tranche_1_20260831"
TRANCHE_ID = "trn_lifecycle_national_1_20260831"
...
def claim_id(priority_id: str, index: int) -> str:
    return stable_id("clm", "national_lifecycle_tranche_1", priority_id, str(index))
...
def build() -> tuple[
    ...
    initial_tranche = load("site/public/data/v1/lifecycle/national-initial-tranche.json")
    baseline_coverage = load("site/public/data/v1/counties/lifecycle-national-expansion-coverage.json")
    final_review = load("data/silver/infrastructure/im3-2026.02.09-final-boundary-review.json")
    source_document = load("config/v1/national-lifecycle-tranche-1-evidence-sources.json")
    adjudication_document = load("config/v1/national-lifecycle-tranche-1-adjudications.json")
    generated_at = adjudication_document["generated_at"]

    candidates_by_id = {record["national_priority_id"]: record for record in initial_tranche}
    facilities_by_id = {
    ...
    if len(adjudications) != 8 or len(adjudication_ids) != len(set(adjudication_ids)):
        raise RuntimeError("The first national tranche must contain eight distinct adjudications")
    if any(priority_id not in candidates_by_id for priority_id in adjudication_ids):
        raise RuntimeError("National adjudication references a candidate outside the initial tranche")
    ...
    if {candidates_by_id[priority_id]["initial_tranche_rank"] for priority_id in adjudication_ids} != expected_ranks:
        raise RuntimeError("The first national tranche must review initial queue ranks one through eight")
    ...
    for adjudication in adjudications:
        ...
        for index, evidence in enumerate(adjudication["evidence"]):
            ...
            current_claim_id = claim_id(priority_id, index)
            claim = {
                ...
                "extractor_version": "national-lifecycle-tranche-1-v1",
            ...
            if adjudication["resolution_state"] == "conflicting":
                claim["conflict_group"] = f"{priority_id}:facility.current_status"
            claims.append(claim)
            evidence_claims.append(claim)

        review_id = stable_id("rvw", "national_lifecycle_tranche_1", priority_id)
        reviews.append(
            {
                "schema_version": "1.0.0",
        ...
        resolution = {
            "schema_version": "1.0.0",
            "resolution_id": stable_id("res", "national_lifecycle_tranche_1", priority_id),
            "subject": {"entity_type": "facility", "entity_id": facility_id},
        ...
        for event_index, event_spec in enumerate(adjudication.get("events", [])):
            ...
            events.append(
                {
                    "schema_version": "1.0.0",
                    "event_id": stable_id("evt", "national_lifecycle_tranche_1", priority_id, str(event_index)),
        ...
        for observation_index, observation_spec in enumerate(adjudication.get("observations", [])):
            ...
            matching_claims = [
                claim["claim_id"]
                for claim in evidence_claims
                if claim["source_id"] == observation_spec["source_id"]
                and claim["attribute_path"] == attribute_path
            ]
            ...
            observations.append(
                {
                    "schema_version": "1.0.0",
                    "observation_id": stable_id("obs", "national_lifecycle_tranche_1", priority_id, str(observation_index)),
    ...
    remaining_queue = sorted(
        [record for record in initial_tranche if record["national_priority_id"] not in reviewed_ids],
    ...
    metadata = {
        ...
        "policy_id": POLICY_ID,
        "input_dataset_ids": [
            "im3_lifecycle_national_priority_20260831",
            "im3_lifecycle_tranche_2_20260831",
        ],
        "counts": {
            "active_canonical_facility_count": sum(record["active_canonical_facility_count"] for record in coverage),
            "initial_tranche_facility_count": len(initial_tranche),
        ...
        "notices": [
            "The first eight national priority records are resolved operational from exact-code, exact-address, operator, or local-government evidence.",
            "The former Digital Realty HVN10 label is retained for continuity, but current municipal evidence identifies different ownership and a prospective tenant.",
            "Equinix colocation-space figures are not published as total building area.",
            "Forty facilities remain queued in the balanced initial national tranche.",
        ],
    }
    document = {
        "schema_version": "1.0.0",
        ...
        "generated_at": generated_at,
        "record_count": sum(
            map(len, [sources, claims, resolutions, reviews, events, observations, updated_facilities])
        ),
        "collections": {
            "source": sources,
            "claim": claims,
...
def main() -> int:
    ...
    outputs = [
        ("data/silver/infrastructure/im3-2026.02.09-lifecycle-national-tranche-1.json", document, True, document["record_count"], "silver", "national_lifecycle_tranche"),
        ("data/silver/infrastructure/im3-2026.02.09-lifecycle-national-tranche-1.processing-report.json", report, False, 1, "silver", "processing_report"),
        ("site/public/data/v1/lifecycle/national-tranche-1-results.json", results, True, len(results), "public", "national_lifecycle_results"),
        ("site/public/data/v1/lifecycle/national-tranche-1-remaining-queue.json", remaining_queue, True, len(remaining_queue), "public", "national_lifecycle_remaining_queue"),
        ("site/public/data/v1/counties/lifecycle-national-tranche-1-coverage.json", coverage, True, len(coverage), "public", "national_lifecycle_coverage"),
        ("site/public/data/v1/lifecycle/national-tranche-1-metadata.json", metadata, False, 1, "public", "national_lifecycle_metadata"),
    ]
    parts = []
    ...
    write_json(
        ROOT / "data/silver/infrastructure/im3-2026.02.09-lifecycle-national-tranche-1.manifest.json",
        manifest,
    )
