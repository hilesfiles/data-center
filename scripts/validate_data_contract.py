...
def validate_project_config(
    ...
    lifecycle_anchor_source_path = CONFIG_DIR / "first-entry-lifecycle-anchor-evidence-sources.json"
    lifecycle_anchor_source_document = load_json(lifecycle_anchor_source_path)
    lifecycle_anchor_sources = lifecycle_anchor_source_document.get("records", [])
    if lifecycle_anchor_source_document.get("record_count") != len(lifecycle_anchor_sources):
        issues.append(Issue("config_validation", lifecycle_anchor_source_path.name, "record count is inconsistent"))
    for index, record in enumerate(lifecycle_anchor_sources):
        for issue in validator.validate_record(record, schema_paths["source"]):
            issues.append(Issue("config_validation", f"{lifecycle_anchor_source_path.name}.records[{index}]{issue.path[1:]}", issue.message))

    lifecycle_anchor_adjudication_path = CONFIG_DIR / "first-entry-lifecycle-anchor-adjudications.json"
    lifecycle_anchor_adjudication_document = load_json(lifecycle_anchor_adjudication_path)
    lifecycle_anchor_adjudications = lifecycle_anchor_adjudication_document.get("records", [])
    lifecycle_anchor_source_ids = {record.get("source_id") for record in lifecycle_anchor_sources}
    if lifecycle_anchor_adjudication_document.get("record_count") != len(lifecycle_anchor_adjudications):
        issues.append(Issue("config_validation", lifecycle_anchor_adjudication_path.name, "record count is inconsistent"))
    for index, record in enumerate(lifecycle_anchor_adjudications):
        for issue in validator.validate_record(record, schema_paths["national_lifecycle_evidence_adjudication"]):
            issues.append(Issue("config_validation", f"{lifecycle_anchor_adjudication_path.name}.records[{index}]{issue.path[1:]}", issue.message))
        referenced_source_ids = {
            evidence.get("source_id") for evidence in record.get("evidence", [])
        } | {event.get("source_id") for event in record.get("events", [])}
        if not referenced_source_ids.issubset(lifecycle_anchor_source_ids):
            issues.append(Issue("referential_integrity", f"{lifecycle_anchor_adjudication_path.name}.records[{index}]", "adjudication references an unknown anchor lifecycle source"))
    ...
    for evidence_path in CONFIG_DIR.glob("*evidence-sources.json"):
        for record in load_json(evidence_path).get("records", []):
            ...
            all_evidence_source_ids.add(source_id)
            previous = all_evidence_source_records.get(source_id)
            if previous is not None and previous != record:
                issues.append(Issue("config_validation", evidence_path.name, f"conflicting source definition {source_id}"))
            all_evidence_source_records[source_id] = record
    ...
    authoritative_resolution_types = {
    ...
    for resolution_tranche_adjudication_path, index, record in resolution_adjudication_locations:
        ...
        resolution_adjudicated_candidate_ids.add(record.get("resolution_candidate_id", ""))
        source_ids = set(record.get("source_ids", []))
        search_rounds = record.get("search_rounds", [])
        round_source_ids = {
            source_id for search_round in search_rounds for source_id in search_round.get("source_ids", [])
        }
        finding_source_ids = {
            source_id for finding in record.get("earlier_operational_findings", [])
            for source_id in finding.get("source_ids", [])
        }
        actual_authoritative_types = {
            all_evidence_source_records[source_id].get("source_type")
            for source_id in source_ids if source_id in all_evidence_source_records
            and all_evidence_source_records[source_id].get("source_type") in authoritative_resolution_types
        }
        if (
            not source_ids.issubset(all_evidence_source_ids)
            or not round_source_ids.issubset(source_ids)
            or not finding_source_ids.issubset(source_ids)
            or [search_round.get("round") for search_round in search_rounds] != [1, 2, 3, 4]
            or [search_round.get("name") for search_round in search_rounds] != expected_round_names
    ...
    new_first_entry_decisions = {
        ...
        "40101": "continue_research",
        "41017": "continue_research",
        "48139": "continue_research",
        "48453": "reject_candidate_as_first_entry",
        "51061": "continue_research",
        "55015": "continue_research",
        "55101": "continue_research",
        "19155": "reject_candidate_as_first_entry",
        "26163": "continue_research",
        "29165": "continue_research",
        "32029": "continue_research",
        "36063": "reject_candidate_as_first_entry",
    ...
    new_first_entry_records = [
        ...
        if record.get("county_fips") in new_first_entry_expected
    ]
    if (
        {record.get("county_fips") for record in new_first_entry_records} != new_first_entry_expected
        or {
            record.get("county_fips"): record.get("decision")
            for record in new_first_entry_records
        } != new_first_entry_decisions
    ):
        issues.append(Issue("config_validation", first_entry_adjudication_path.name, "new tranche adjudication outcomes are inconsistent"))
    first_entry_adjudications = [
        record for record in first_entry_adjudications
        if record.get("county_fips") not in new_first_entry_expected
    ]
    if (
        {record.get("county_fips") for record in first_entry_adjudications} != {"01069", "04013", "05119", "05145", "06037", "06055", "06079", "06085", "06095", "06099", "08001", "11001", "12001", "12057", "13067", "13117", "13121", "17019", "17031", "17043", "18089", "18105", "21071", "23005", "24005", "24031", "24033", "25017", "26125", "27019", "27109", "27123", "27137", "29019", "30017", "31153", "32003", "33015", "34003", "34017", "34023", "34039", "35001", "35049", "36047", "36067", "36085", "36087", "37119", "38017", "39017", "39023", "39045", "39153", "39165", "40013", "40109", "41005", "41047", "41051", "42003", "42077", "42091", "42101", "45051", "46099", "47145", "47157", "48085", "48325", "48475", "49047", "51107", "53033", "53061", "01071", "06059", "48113", "53073", "55133", "12011", "12031", "21111", "24003", "26045", "34013", "08031", "13135", "29510", "34031", "41059", "48029", "48121", "53053", "36001", "36061", "37161", "47165", "53063", "55079", "13215", "13097", "37183", "39035", "47125", "51087", "29047", "01089", "06001", "06067", "19181", "24510", "35061", "47187", "49035", "53025", "06073", "27053", "40143", "08035", "12095", "29095", "36029", "39061", "48339", "55025", "06075", "08041", "12086", "48439", "19049", "39159", "45015", "49049", "04003", "04019", "24021", "24027", "25009", "26049", "33017"}
        or {
            record.get("county_fips"): record.get("decision")
            for record in first_entry_adjudications
        } != {
            "01069": "continue_research",
            "04013": "reject_candidate_as_first_entry",
            "05119": "reject_candidate_as_first_entry",
            "05145": "continue_research",
            "06037": "reject_candidate_as_first_entry",
            "06055": "continue_research",
            "06079": "reject_candidate_as_first_entry",
            "06085": "reject_candidate_as_first_entry",
            "06095": "continue_research",
            "06099": "reject_candidate_as_first_entry",
            "08001": "continue_research",
            "11001": "reject_candidate_as_first_entry",
            "12001": "reject_candidate_as_first_entry",
            "12057": "reject_candidate_as_first_entry",
            "13067": "reject_candidate_as_first_entry",
            "13117": "reject_candidate_as_first_entry",
            "13121": "reject_candidate_as_first_entry",
            "17019": "reject_candidate_as_first_entry",
            "17031": "reject_candidate_as_first_entry",
            "17043": "reject_candidate_as_first_entry",
            "18089": "continue_research",
            "18105": "reject_candidate_as_first_entry",
            "21071": "continue_research",
            "23005": "reject_candidate_as_first_entry",
            "24005": "reject_candidate_as_first_entry",
            "24031": "reject_candidate_as_first_entry",
            "24033": "reject_candidate_as_first_entry",
            "25017": "reject_candidate_as_first_entry",
            "26125": "reject_candidate_as_first_entry",
            "27019": "reject_candidate_as_first_entry",
            "27109": "reject_candidate_as_first_entry",
            "27123": "reject_candidate_as_first_entry",
            "27137": "reject_candidate_as_first_entry",
            "29019": "reject_candidate_as_first_entry",
            "30017": "continue_research",
            "31153": "reject_candidate_as_first_entry",
            "32003": "reject_candidate_as_first_entry",
            "33015": "continue_research",
            "34003": "reject_candidate_as_first_entry",
            "34017": "continue_research",
            "34023": "reject_candidate_as_first_entry",
            "34039": "continue_research",
            "35001": "reject_candidate_as_first_entry",
            "35049": "continue_research",
            "36047": "continue_research",
            "36067": "reject_candidate_as_first_entry",
            "36087": "reject_candidate_as_first_entry",
            "37119": "reject_candidate_as_first_entry",
            "38017": "reject_candidate_as_first_entry",
            "39017": "continue_research",
            "39023": "reject_candidate_as_first_entry",
            "39045": "continue_research",
            "39153": "reject_candidate_as_first_entry",
            "40013": "continue_research",
            "40109": "reject_candidate_as_first_entry",
            "41005": "continue_research",
            "42091": "reject_candidate_as_first_entry",
            "42101": "reject_candidate_as_first_entry",
            "45051": "continue_research",
            "47145": "reject_candidate_as_first_entry",
            "48085": "reject_candidate_as_first_entry",
            "48325": "reject_candidate_as_first_entry",
            "48475": "continue_research",
            "49047": "continue_research",
            "51107": "reject_candidate_as_first_entry",
            "53033": "reject_candidate_as_first_entry",
            "53061": "reject_candidate_as_first_entry",
            "01071": "continue_research",
            "06059": "reject_candidate_as_first_entry",
            "48113": "reject_candidate_as_first_entry",
            "53073": "reject_candidate_as_first_entry",
            "55133": "continue_research",
            "12011": "reject_candidate_as_first_entry",
            "12031": "continue_research",
            "21111": "continue_research",
            "24003": "continue_research",
            "26045": "continue_research",
            "34013": "continue_research",
            "36085": "continue_research",
            "39165": "continue_research",
            "41047": "continue_research",
            "41051": "continue_research",
            "42003": "continue_research",
            "42077": "continue_research",
            "46099": "continue_research",
            "47157": "continue_research",
            "08031": "continue_research",
            "13135": "continue_research",
            "29510": "continue_research",
            "34031": "continue_research",
            "41059": "continue_research",
            "48029": "continue_research",
            "48121": "continue_research",
            "53053": "continue_research",
            "36001": "continue_research",
            "36061": "continue_research",
            "37161": "continue_research",
            "47165": "continue_research",
            "53063": "continue_research",
            "55079": "continue_research",
            "13215": "continue_research",
            "13097": "continue_research",
            "37183": "continue_research",
            "39035": "continue_research",
            "47125": "continue_research",
            "51087": "continue_research",
            "29047": "continue_research",
            "01089": "continue_research",
            "06001": "continue_research",
            "06067": "continue_research",
            "19181": "continue_research",
            "24510": "continue_research",
            "35061": "continue_research",
            "47187": "continue_research",
            "49035": "continue_research",
            "53025": "continue_research",
            "06073": "continue_research",
            "27053": "continue_research",
            "40143": "continue_research",
            "08035": "continue_research",
            "12095": "continue_research",
            "29095": "continue_research",
            "36029": "continue_research",
            "39061": "continue_research",
            "48339": "continue_research",
            "55025": "continue_research",
            "06075": "reject_candidate_as_first_entry",
            "08041": "continue_research",
            "12086": "continue_research",
            "48439": "continue_research",
            "19049": "continue_research",
            "39159": "continue_research",
            "45015": "continue_research",
            "49049": "continue_research",
            "04003": "continue_research",
            "04019": "continue_research",
            "24021": "continue_research",
            "24027": "continue_research",
            "25009": "continue_research",
            "26049": "continue_research",
            "33017": "continue_research",
        }
...
def validate_public_data(
    ...
    new_adjudicated_assessments = [
        record for record in adjudicated_assessments
        if record.get("county_fips") in new_adjudicated_expected
    ]
    if (
        {record.get("county_fips") for record in new_adjudicated_assessments} != new_adjudicated_expected
        or {
            record.get("county_fips"): record.get("candidate_rejection_count")
            for record in new_adjudicated_assessments
        } != new_adjudicated_rejections
    ):
        issues.append(Issue("public_data_validation", treatment_path.name, "new tranche first-entry adjudication summaries are inconsistent"))
    adjudicated_assessments = [
        record for record in adjudicated_assessments
        if record.get("county_fips") not in new_adjudicated_expected
    ]
    if (
        {record.get("county_fips") for record in adjudicated_assessments} != {"01069", "04013", "05119", "05145", "06037", "06055", "06079", "06085", "06095", "06099", "08001", "11001", "12001", "12057", "13067", "13117", "13121", "17019", "17031", "17043", "18089", "18105", "21071", "23005", "24005", "24031", "24033", "25017", "26125", "27019", "27109", "27123", "27137", "29019", "30017", "31153", "32003", "33015", "34003", "34017", "34023", "34039", "35001", "35049", "36047", "36067", "36085", "36087", "37119", "38017", "39017", "39023", "39045", "39153", "39165", "40013", "40109", "41005", "41047", "41051", "42003", "42077", "42091", "42101", "45051", "46099", "47145", "47157", "48085", "48325", "48475", "49047", "51107", "53033", "53061", "01071", "06059", "48113", "53073", "55133", "12011", "12031", "21111", "24003", "26045", "34013", "08031", "13135", "29510", "34031", "41059", "48029", "48121", "53053", "36001", "36061", "37161", "47165", "53063", "55079", "13215", "13097", "37183", "39035", "47125", "51087", "29047", "01089", "06001", "06067", "19181", "24510", "35061", "47187", "49035", "53025", "06073", "27053", "40143", "08035", "12095", "29095", "36029", "39061", "48339", "55025", "06075", "08041", "12086", "48439", "19049", "39159", "45015", "49049", "04003", "04019", "24021", "24027", "25009", "26049", "33017"}
        or {
            record.get("county_fips"): record.get("candidate_rejection_count")
            for record in adjudicated_assessments
        } != {"01069": 0, "04013": 1, "05119": 1, "05145": 0, "06037": 1, "06055": 0, "06079": 1, "06085": 1, "06095": 0, "06099": 1, "08001": 0, "11001": 1, "12001": 1, "12057": 1, "13067": 1, "13117": 1, "13121": 1, "17019": 1, "17031": 1, "17043": 1, "18089": 0, "18105": 1, "21071": 0, "23005": 1, "24005": 1, "24031": 1, "24033": 1, "25017": 1, "26125": 1, "27019": 1, "27109": 1, "27123": 1, "27137": 1, "29019": 1, "30017": 0, "31153": 1, "32003": 1, "33015": 0, "34003": 1, "34017": 0, "34023": 1, "34039": 0, "35001": 1, "35049": 0, "36047": 0, "36067": 1, "36085": 0, "36087": 1, "37119": 1, "38017": 1, "39017": 0, "39023": 1, "39045": 0, "39153": 1, "39165": 0, "40013": 0, "40109": 1, "41005": 0, "41047": 0, "41051": 0, "42003": 0, "42077": 0, "42091": 1, "42101": 1, "45051": 0, "46099": 0, "47145": 1, "47157": 0, "48085": 1, "48325": 1, "48475": 0, "49047": 0, "51107": 1, "53033": 1, "53061": 1, "01071": 0, "06059": 1, "48113": 1, "53073": 1, "55133": 0, "12011": 1, "12031": 0, "21111": 0, "24003": 0, "26045": 0, "34013": 0, "08031": 0, "13135": 0, "29510": 0, "34031": 0, "41059": 0, "48029": 0, "48121": 0, "53053": 0, "36001": 0, "36061": 0, "37161": 0, "47165": 0, "53063": 0, "55079": 0, "13215": 0, "13097": 0, "37183": 0, "39035": 0, "47125": 0, "51087": 0, "29047": 0, "01089": 0, "06001": 0, "06067": 0, "19181": 0, "24510": 0, "35061": 0, "47187": 0, "49035": 0, "53025": 0, "06073": 0, "27053": 0, "40143": 0, "08035": 0, "12095": 0, "29095": 0, "36029": 0, "39061": 0, "48339": 0, "55025": 0, "06075": 1, "08041": 0, "12086": 0, "48439": 0, "19049": 0, "39159": 0, "45015": 0, "49049": 0, "04003": 0, "04019": 0, "24021": 0, "24027": 0, "25009": 0, "26049": 0, "33017": 0}
    ...
    expected_treatment_evaluations = {
        ...
        "12001": {
            "facility_id": "fac_im3_building_00429782814",
            "source_id": "src_uf_research_computing_history_current_2026",
            "when": {"year": 2012, "precision": "year"},
            "data_quality_score": 76.05,
        ...
        "23005": {
            "facility_id": "fac_im3_building_00667796922",
            "source_id": "src_bdn_oxford_resilient_open_20110928",
            "when": {"date": "2011-09-28", "precision": "day"},
            "data_quality_score": 86.4,
            "available_pre_periods": 10,
            "available_post_periods": 13,
            "evidence_threshold_status": "failed",
            "period_requirement_status": "passed",
            "exclusion_reasons": [
                "evidence_threshold_not_met",
        ...
        "25017": {
            "facility_id": "fac_im3_building_00214321737",
            "source_id": "src_markley_lowell_phase_one_open_20151123",
            "when": {"date": "2015-11-23", "precision": "day"},
        ...
        "12057": {
            "facility_id": "fac_im3_building_00523886201",
            "source_id": "src_florida_edr_datacenter_inventory_20160120",
            "when": {"date": "2016-01-20", "precision": "day"},
            "data_quality_score": 97.02,
        ...
        "13067": {
            "facility_id": "fac_im3_building_00284044002",
            "source_id": "src_tradeline_southtrust_marietta_completion_20020711",
            "when": {"date": "2002-05-01", "precision": "month"},
            "data_quality_score": 83.79,
        ...
        "42091": {
            "facility_id": "fac_im3_building_01109881281",
            "source_id": "src_kentucky_psc_dbsi_valley_forge_20130430",
            "when": {"date": "2013-04-30", "precision": "day"},
            "data_quality_score": 97.02,
            "available_pre_periods": 12,
            "available_post_periods": 11,
            "evidence_threshold_status": "passed",
            "period_requirement_status": "passed",
            "exclusion_reasons": ["candidate_event_not_county_first_entry"],
        },
        "37119": {
            "facility_id": "fac_im3_building_00838817907",
            "source_id": "src_windstream_charlotte_four_opening_20140303",
            "when": {"date": "2014-03-03", "precision": "day"},
        ...
        "33015": {
            "facility_id": "fac_im3_building_00589070714",
            "source_id": "src_exeter_ebook_2008_2009",
            "when": {"date": "2009-06-30", "precision": "day"},
            "data_quality_score": 89.28,
            "available_pre_periods": 8,
            "available_post_periods": 15,
            "evidence_threshold_status": "passed",
            "period_requirement_status": "passed",
    ...
    expected_treatment_evaluations.update({
        "06079": {
            "facility_id": "fac_im3_point_08477460442",
            "source_id": "src_digitalwest_soc2_audit_20130424",
            "when": {"date": "2013-04-24", "precision": "day"},
            "data_quality_score": 94.08,
            "available_pre_periods": 12,
            "available_post_periods": 11,
            "evidence_threshold_status": "passed",
            "period_requirement_status": "passed",
        ...
        "06095": {
            "facility_id": "fac_im3_building_00780063735",
            "source_id": "src_cpuc_pge_nielsen_425_beck_2021",
            "when": {"year": 2006, "precision": "year"},
            "data_quality_score": 68.29,
            "available_pre_periods": 5,
            "available_post_periods": 18,
            "evidence_threshold_status": "failed",
            "period_requirement_status": "failed",
            "exclusion_reasons": ["evidence_threshold_not_met", "panel_period_requirement_not_met", "county_first_entry_not_verified"],
        },
        "06099": {
            "facility_id": "fac_im3_building_01296163129",
            "source_id": "src_yccd_primary_data_center_2013",
            "when": {"year": 2013, "precision": "year"},
            "data_quality_score": 75.24,
            "available_pre_periods": 12,
            "available_post_periods": 11,
            "evidence_threshold_status": "failed",
            "period_requirement_status": "passed",
            "exclusion_reasons": ["evidence_threshold_not_met", "candidate_event_not_county_first_entry"],
        },
        "08001": {
            "facility_id": "fac_im3_building_00426586135",
            "source_id": "src_doubleclick_epsilon_thornton_lease_20070201",
            "when": {"date": "2007-02-01", "precision": "day"},
            "data_quality_score": 97.02,
            "available_pre_periods": 6,
            "available_post_periods": 17,
            "evidence_threshold_status": "passed",
            "period_requirement_status": "failed",
            "exclusion_reasons": ["panel_period_requirement_not_met", "county_first_entry_not_verified"],
        },
        "11001": {
            "facility_id": "fac_im3_point_00751881301",
            "source_id": "src_sec_coresite_1275_k_2010",
            "when": {"year": 2006, "precision": "year"},
            "data_quality_score": 77.62,
            "available_pre_periods": 5,
            "available_post_periods": 18,
            "evidence_threshold_status": "failed",
            "period_requirement_status": "failed",
            "exclusion_reasons": ["evidence_threshold_not_met", "panel_period_requirement_not_met", "candidate_event_not_county_first_entry"],
        },
        "13117": {
            "facility_id": "fac_im3_building_01282332209",
            "source_id": "src_forsyth_economic_plan_data_centers_2019",
            "when": {"year": 2019, "precision": "year"},
            "data_quality_score": 70.68,
            "available_pre_periods": 18,
            "available_post_periods": 5,
            "evidence_threshold_status": "failed",
            "period_requirement_status": "passed",
            "exclusion_reasons": ["evidence_threshold_not_met", "candidate_event_not_county_first_entry"],
        },
        "01069": {
            "facility_id": "fac_im3_building_00626281456",
            "source_id": "src_sourcewell_angeltrax_contract_20250422",
            "when": {"date": "2025-04-22", "precision": "day"},
            "data_quality_score": 90.25,
        ...
        "05119": {
            "facility_id": "fac_im3_building_00388148510",
            "source_id": "src_windstream_little_rock_opening_20120412",
            "when": {"date": "2012-04-12", "precision": "day"},
            "data_quality_score": 97.02,
            "available_pre_periods": 11,
            "available_post_periods": 12,
            "evidence_threshold_status": "passed",
        ...
        "05145": {
            "facility_id": "fac_im3_building_01020721870",
            "source_id": "src_searcy_it_data_center_current_2026",
            "when": {"date": "2026-09-01", "precision": "day"},
            "data_quality_score": 87.4,
            "available_pre_periods": 24,
            "available_post_periods": 0,
            "evidence_threshold_status": "passed",
            "period_requirement_status": "failed",
        ...
        "06055": {
            "facility_id": "fac_im3_building_00464371693",
            "source_id": "src_kaiser_napa_green_it_20111024",
            "when": {"date": "2011-10-24", "precision": "day"},
            "data_quality_score": 96.04,
            "available_pre_periods": 10,
            "available_post_periods": 13,
            "evidence_threshold_status": "passed",
        ...
        "34023": {
            "facility_id": "fac_im3_building_00362662493",
            "source_id": "src_qts_2016_annual_report_piscataway_acquisition",
            "when": {"date": "2016-06-06", "precision": "day"},
            "data_quality_score": 96.04,
            "available_pre_periods": 15,
            "available_post_periods": 8,
            "evidence_threshold_status": "passed",
        ...
        "48085": {
            "facility_id": "fac_im3_building_00254773897",
            "source_id": "src_evoque_allen_da1_brief_2018",
            "when": {"year": 1999, "precision": "year"},
            "data_quality_score": 70.56,
            "available_pre_periods": 0,
            "available_post_periods": 24,
            "evidence_threshold_status": "failed",
            "period_requirement_status": "failed",
            "exclusion_reasons": ["evidence_threshold_not_met", "panel_period_requirement_not_met", "candidate_event_not_county_first_entry"],
        },
        "53033": {
            "facility_id": "fac_im3_building_00359930465",
            "source_id": "src_equinix_se3_opening_20130314",
            "when": {"date": "2013-03-14", "precision": "day"},
            "data_quality_score": 97.02,
            "available_pre_periods": 12,
            "available_post_periods": 11,
            "evidence_threshold_status": "passed",
        ...
        "36087": {
            "facility_id": "fac_im3_building_01238955720",
            "source_id": "src_orangetown_bloomberg_agenda_20181023",
            "when": {"date": "2018-10-23", "precision": "day"},
            "data_quality_score": 90.25,
            "available_pre_periods": 17,
            "available_post_periods": 6,
            "evidence_threshold_status": "passed",
            "period_requirement_status": "passed",
            "exclusion_reasons": ["candidate_event_not_county_first_entry"],
        },
        "53061": {
            "facility_id": "fac_im3_building_00396296733",
            "source_id": "src_csquare_seattle_specsheet_2025",
            "when": {"date": "2025-12-31", "precision": "day"},
            "data_quality_score": 96.04,
            "available_pre_periods": 24,
            "available_post_periods": 0,
            "evidence_threshold_status": "passed",
        ...
        "51107": {
            "facility_id": "fac_im3_building_00048242814",
            "source_id": "src_loudoun_iad10_permit_202404",
            "when": {"date": "2024-04-30", "precision": "day"},
            "data_quality_score": 90.25,
            "available_pre_periods": 23,
            "available_post_periods": 0,
            "evidence_threshold_status": "passed",
            "period_requirement_status": "failed",
            "exclusion_reasons": ["panel_period_requirement_not_met", "candidate_event_not_county_first_entry"],
        ...
        "35001": {
            "facility_id": "fac_im3_building_00014930068",
            "source_id": "src_sandia_building_880_family_day_2019",
            "when": {"year": 2019, "precision": "year"},
            "data_quality_score": 70.56,
            "available_pre_periods": 18,
            "available_post_periods": 5,
        ...
        "31153": {
            "facility_id": "fac_im3_building_00421273622",
            "source_id": "src_papillion_fidelity_permit_2025",
            "when": {"date": "2025-08-05", "precision": "day"},
            "data_quality_score": 92.15,
            "available_pre_periods": 24,
            "available_post_periods": 0,
            "evidence_threshold_status": "passed",
            "period_requirement_status": "failed",
        ...
        "06037": {
            "facility_id": "fac_im3_building_00424945845",
            "source_id": "src_serverfarm_lax1_acquisition_20210420",
            "when": {"date": "2021-04-20", "precision": "day"},
            "data_quality_score": 96.04,
            "available_pre_periods": 20,
            "available_post_periods": 3,
            "evidence_threshold_status": "passed",
        ...
        "17043": {
            "facility_id": "fac_im3_building_00377212693",
            "source_id": "src_element_critical_chicago_acquisition_20190108",
            "when": {"date": "2019-01-08", "precision": "day"},
            "data_quality_score": 96.04,
            "available_pre_periods": 18,
            "available_post_periods": 5,
            "evidence_threshold_status": "passed",
        ...
        "18089": {
            "facility_id": "fac_im3_building_00978934687",
            "source_id": "src_t5_digital_crossroad_launch_20200825",
            "when": {"date": "2020-10-31", "precision": "day"},
            "data_quality_score": 96.04,
            "available_pre_periods": 19,
            "available_post_periods": 4,
            "evidence_threshold_status": "passed",
            "period_requirement_status": "passed",
            "exclusion_reasons": ["county_first_entry_not_verified"],
        },
        "17019": {
            "facility_id": "fac_im3_building_00138260136",
            "source_id": "src_ncsa_npcf_completion_20100402",
            "when": {"date": "2010-06-01", "precision": "day"},
            "data_quality_score": 98.01,
        ...
        "24005": {
            "facility_id": "fac_im3_point_11167641842",
            "source_id": "src_expedient_owings_mills_opening_20130528",
            "when": {"date": "2013-11-01", "precision": "month"},
        ...
        "24031": {
            "facility_id": "fac_im3_building_00093210344",
            "source_id": "src_federal_register_bytegrid_vdei_20150706",
            "when": {"date": "2015-07-06", "precision": "day"},
            "data_quality_score": 98.01,
            "available_pre_periods": 14,
            "available_post_periods": 9,
            "evidence_threshold_status": "passed",
        ...
        "24033": {
            "facility_id": "fac_im3_building_00363017375",
            "source_id": "src_pgcounty_known_data_centers_20250625",
            "when": {"date": "2004-04-01", "precision": "day"},
            "data_quality_score": 98.01,
            "available_pre_periods": 3,
            "available_post_periods": 20,
            "evidence_threshold_status": "passed",
            "period_requirement_status": "failed",
            "exclusion_reasons": ["panel_period_requirement_not_met", "candidate_event_not_county_first_entry"],
        },
        "27019": {
            "facility_id": "fac_im3_building_00052227492",
            "source_id": "src_stream_chaska_history_current_2026",
            "when": {"date": "2014-06-01", "precision": "month"},
        ...
        "27109": {
            "facility_id": "fac_im3_building_00718465287",
            "source_id": "src_finance_commerce_mayo_epic_20160104",
            "when": {"year": 2012, "precision": "year"},
            "data_quality_score": 68.45,
            "available_pre_periods": 11,
            "available_post_periods": 12,
            "evidence_threshold_status": "failed",
            "period_requirement_status": "passed",
        ...
        "27123": {
            "facility_id": "fac_im3_building_00131874718",
            "source_id": "src_h5_vxchnge_acquisition_20210131",
            "when": {"date": "2021-01-31", "precision": "day"},
            "data_quality_score": 98.01,
            "available_pre_periods": 20,
            "available_post_periods": 3,
            "evidence_threshold_status": "passed",
        ...
        "27137": {
            "facility_id": "fac_im3_building_01307072785",
            "source_id": "src_ark_duluth_apex_history_current_2026",
            "when": {"date": "2012-09-01", "precision": "month"},
            "data_quality_score": 90.31,
            "available_pre_periods": 11,
            "available_post_periods": 12,
            "evidence_threshold_status": "passed",
            "period_requirement_status": "passed",
        ...
        "29019": {
            "facility_id": "fac_im3_building_00553884511",
            "source_id": "src_dcd_carfax_columbia_20241002",
            "when": {"year": 2004, "precision": "year"},
            "data_quality_score": 67.68,
            "available_pre_periods": 3,
            "available_post_periods": 20,
            "evidence_threshold_status": "failed",
            "period_requirement_status": "failed",
        ...
        "30017": {
            "facility_id": "fac_im3_building_01417637306",
            "source_id": "src_montana_biennial_miles_city_2015",
            "when": {"date": "2012-04-01", "precision": "month"},
            "data_quality_score": 93.11,
            "available_pre_periods": 11,
            "available_post_periods": 12,
            "evidence_threshold_status": "passed",
        ...
        "34003": {
            "facility_id": "fac_im3_building_00472761713",
            "source_id": "src_nyse_2012_10k_mahwah_operational",
            "when": {"year": 2010, "precision": "year"},
            "data_quality_score": 78.41,
            "available_pre_periods": 9,
            "available_post_periods": 14,
            "evidence_threshold_status": "failed",
            "period_requirement_status": "passed",
        ...
        "21071": {
            "facility_id": "fac_im3_building_01456953254",
            "source_id": "src_idi_ekn_data_center_20230501",
            "when": {"date": "2023-05-01", "precision": "day"},
            "data_quality_score": 96.04,
            "available_pre_periods": 22,
            "available_post_periods": 1,
            "evidence_threshold_status": "passed",
            "period_requirement_status": "failed",
            "exclusion_reasons": ["panel_period_requirement_not_met", "county_first_entry_not_verified"],
        },
        "34039": {
            "facility_id": "fac_im3_building_00015884451",
            "source_id": "src_njbpu_barclays_cranford_20211117",
            "when": {"date": "2020-06-19", "precision": "day"},
            "data_quality_score": 98.01,
            "available_pre_periods": 19,
            "available_post_periods": 4,
            "evidence_threshold_status": "passed",
        ...
        "35049": {
            "facility_id": "fac_im3_building_00318069356",
            "source_id": "src_nm_lfc_simms_data_center_fy2008",
            "when": {"year": 2007, "precision": "year"},
            "data_quality_score": 78.41,
            "available_pre_periods": 6,
            "available_post_periods": 17,
            "evidence_threshold_status": "failed",
        ...
        "36047": {
            "facility_id": "fac_im3_point_12766117170",
            "source_id": "src_dcd_dataverge_brooklyn_20240110",
            "when": {"year": 2003, "precision": "year"},
            "data_quality_score": 69.18,
            "available_pre_periods": 2,
            "available_post_periods": 21,
            "evidence_threshold_status": "failed",
            "period_requirement_status": "failed",
        ...
        "36067": {
            "facility_id": "fac_im3_building_00930043986",
            "source_id": "src_su_archives_green_data_center_current_2026",
            "when": {"date": "2009-12-02", "precision": "day"},
            "data_quality_score": 98.01,
            "available_pre_periods": 8,
            "available_post_periods": 15,
            "evidence_threshold_status": "passed",
        ...
        "38017": {
            "facility_id": "fac_im3_building_00019390019",
            "source_id": "src_dcn_fargo_history_20190819",
            "when": {"year": 2006, "precision": "year"},
            "data_quality_score": 76.03,
            "available_pre_periods": 5,
            "available_post_periods": 18,
            "evidence_threshold_status": "failed",
            "period_requirement_status": "failed",
        ...
        "39017": {
            "facility_id": "fac_im3_point_12112201416",
            "source_id": "src_west_chester_peak10_2014",
            "when": {"year": 2014, "precision": "year"},
            "data_quality_score": 78.41,
            "available_pre_periods": 13,
            "available_post_periods": 10,
            "evidence_threshold_status": "failed",
        ...
        "39023": {
            "facility_id": "fac_im3_building_00138643160",
            "source_id": "src_dcd_lexis_springfield_20070128",
            "when": {"year": 2005, "precision": "year"},
            "data_quality_score": 70.69,
            "available_pre_periods": 4,
            "available_post_periods": 19,
            "evidence_threshold_status": "failed",
            "period_requirement_status": "failed",
            "exclusion_reasons": ["evidence_threshold_not_met", "panel_period_requirement_not_met", "candidate_event_not_county_first_entry"],
        },
        "39045": {
            "facility_id": "fac_im3_building_01252196824",
            "source_id": "src_lancaster_acfr_2023_google_operational",
            "when": {"year": 2023, "precision": "year"},
            "data_quality_score": 78.41,
            "available_pre_periods": 22,
            "available_post_periods": 1,
            "evidence_threshold_status": "failed",
        ...
        "39153": {
            "facility_id": "fac_im3_building_01278541792",
            "source_id": "src_summit_involta_opened_2012",
            "when": {"year": 2012, "precision": "year"},
            "data_quality_score": 78.41,
            "available_pre_periods": 11,
            "available_post_periods": 12,
        ...
        "40013": {
            "facility_id": "fac_im3_building_00726150321",
            "source_id": "src_choctaw_hq_move_201804",
            "when": {"year": 2018, "precision": "year"},
            "data_quality_score": 73.7,
            "available_pre_periods": 17,
            "available_post_periods": 6,
            "evidence_threshold_status": "failed",
            "period_requirement_status": "passed",
        ...
        "40109": {
            "facility_id": "fac_im3_point_06401035673",
            "source_id": "src_rack59_about_current_2026",
            "when": {"year": 2009, "precision": "year"},
            "data_quality_score": 78.41,
            "available_pre_periods": 8,
            "available_post_periods": 15,
        ...
        "41005": {
            "facility_id": "fac_im3_building_00480762078",
            "source_id": "src_plannet_mentor_wilsonville_data_center_20121205",
            "when": {"year": 2012, "precision": "approximate_year"},
            "data_quality_score": 57.43,
            "available_pre_periods": 11,
            "available_post_periods": 12,
            "evidence_threshold_status": "failed",
            "period_requirement_status": "passed",
        ...
        "42101": {
            "facility_id": "fac_im3_building_00334264478",
            "source_id": "src_philadelphia_401nbroad_permit_archive",
            "when": {"year": 1998, "precision": "year"},
            "data_quality_score": 74.48,
            "available_pre_periods": 0,
            "available_post_periods": 24,
            "evidence_threshold_status": "failed",
            "period_requirement_status": "failed",
        ...
        "45051": {
            "facility_id": "fac_im3_building_01320038649",
            "source_id": "src_dcblox_myrtle_open_20231010",
            "when": {"date": "2023-10-10", "precision": "day"},
            "data_quality_score": 98.01,
            "available_pre_periods": 22,
            "available_post_periods": 1,
            "evidence_threshold_status": "passed",
        ...
        "47145": {
            "facility_id": "fac_im3_point_10648552506",
            "source_id": "src_ornl_frontier_launch_20220817",
            "when": {"date": "2022-08-17", "precision": "day"},
            "data_quality_score": 98.01,
            "available_pre_periods": 21,
            "available_post_periods": 2,
            "evidence_threshold_status": "passed",
            "period_requirement_status": "failed",
            "exclusion_reasons": ["panel_period_requirement_not_met", "candidate_event_not_county_first_entry"],
        },
        "48325": {
            "facility_id": "fac_im3_building_01010260675",
            "source_id": "src_baxtel_microsoft_trp3_current_2026",
            "when": {"year": 2022, "precision": "year"},
            "data_quality_score": 64.8,
            "available_pre_periods": 21,
            "available_post_periods": 2,
            "evidence_threshold_status": "failed",
            "period_requirement_status": "failed",
        ...
        "48475": {
            "facility_id": "fac_im3_building_01455361421",
            "source_id": "src_sec_ionic_q2_2026",
            "when": {"year": 2023, "precision": "year"},
            "data_quality_score": 78.41,
            "available_pre_periods": 22,
            "available_post_periods": 1,
        ...
        "49047": {
            "facility_id": "fac_im3_building_00611308188",
            "source_id": "src_americanfork_strata_proposal_20220517",
            "when": {"date": "2022-05-17", "precision": "day"},
            "data_quality_score": 96.04,
            "available_pre_periods": 21,
            "available_post_periods": 2,
            "evidence_threshold_status": "passed",
        ...
        "53073": {
            "facility_id": "fac_im3_building_00376627431",
            "source_id": "src_astound_data_center_list_20231219",
            "when": {"date": "2023-12-19", "precision": "day"},
            "data_quality_score": 93.12,
            "available_pre_periods": 22,
            "available_post_periods": 1,
            "evidence_threshold_status": "passed",
            "period_requirement_status": "failed",
        ...
        "55133": {
            "facility_id": "fac_im3_building_01011125238",
            "source_id": "src_pewaukee_we_data_center_minutes_20130516",
            "when": {"date": "2013-05-16", "precision": "day"},
            "data_quality_score": 98.01,
            "available_pre_periods": 12,
            "available_post_periods": 11,
            "evidence_threshold_status": "passed",
        ...
        "48113": {
            "facility_id": "fac_im3_building_00028544618",
            "source_id": "src_equinix_infomart_history_20200604",
            "when": {"year": 2000, "precision": "year"},
            "data_quality_score": 78.41,
            "available_pre_periods": 0,
            "available_post_periods": 24,
            "evidence_threshold_status": "failed",
        ...
        "01071": {
            "facility_id": "fac_im3_building_00844371952",
            "source_id": "src_google_alabama_location_current_2026",
            "when": {"year": 2018, "precision": "year"},
            "data_quality_score": 78.41,
            "available_pre_periods": 17,
            "available_post_periods": 6,
        ...
        "12011": {
            "facility_id": "fac_im3_building_00741629998",
            "source_id": "src_flexential_florida_hurricane_matthew_20161018",
            "when": {"date": "2016-10-18", "precision": "day"},
            "data_quality_score": 96.04,
            "available_pre_periods": 15,
            "available_post_periods": 8,
            "evidence_threshold_status": "passed",
        ...
        "12031": {
            "facility_id": "fac_im3_building_00497151852",
            "source_id": "src_cologix_jax1_upgrade_20151029",
            "when": {"date": "2013-09-01", "precision": "month"},
            "data_quality_score": 92.17,
            "available_pre_periods": 12,
            "available_post_periods": 11,
            "evidence_threshold_status": "passed",
        ...
        "21111": {
            "facility_id": "fac_im3_building_00329551673",
            "source_id": "src_flexential_louisville_acquisition_20170109",
            "when": {"date": "2017-01-09", "precision": "day"},
            "data_quality_score": 96.04,
            "available_pre_periods": 16,
            "available_post_periods": 7,
            "evidence_threshold_status": "passed",
        ...
        "24003": {
            "facility_id": "fac_im3_building_00016282459",
            "source_id": "src_dck_cybernap_conversion_20130613",
            "when": {"year": 2012, "precision": "year"},
            "data_quality_score": 66.24,
            "available_pre_periods": 11,
            "available_post_periods": 12,
            "evidence_threshold_status": "failed",
            "period_requirement_status": "passed",
        ...
        "26045": {
            "facility_id": "fac_im3_building_00544536822",
            "source_id": "src_liquid_web_lansing_current_2026",
            "when": {"year": 1997, "precision": "year"},
            "data_quality_score": 76.83,
            "available_pre_periods": 0,
            "available_post_periods": 24,
            "evidence_threshold_status": "failed",
            "period_requirement_status": "failed",
        ...
        "34013": {
            "facility_id": "fac_im3_building_00387208881",
            "source_id": "src_165_halsey_anniversary_20240318",
            "when": {"year": 1999, "precision": "year"},
            "data_quality_score": 77.62,
            "available_pre_periods": 0,
            "available_post_periods": 24,
        ...
        "06059": {
            "facility_id": "fac_im3_building_00492699069",
            "source_id": "src_tpx_ocix_acquisition_20110531",
            "when": {"date": "2011-05-31", "precision": "day"},
            "data_quality_score": 95.06,
            "available_pre_periods": 10,
            "available_post_periods": 13,
            "evidence_threshold_status": "passed",
        ...
        "36085": {
            "facility_id": "fac_im3_building_00247428943",
            "source_id": "src_telehouse_history_1989",
            "when": {"year": 1989, "precision": "year"},
            "data_quality_score": 78.41,
            "available_pre_periods": 0,
            "available_post_periods": 24,
            "evidence_threshold_status": "failed",
        ...
        "39165": {
            "facility_id": "fac_im3_building_00031250931",
            "source_id": "src_sec_cyrusone_contribution_20121120",
            "when": {"date": "2012-11-20", "precision": "day"},
            "data_quality_score": 98.01,
            "available_pre_periods": 11,
            "available_post_periods": 12,
            "evidence_threshold_status": "passed",
        ...
        "41047": {
            "facility_id": "fac_im3_building_00733721782",
            "source_id": "src_oregon_sos_state_dc_audit_201811",
            "when": {"date": "2008-07-01", "precision": "month"},
            "data_quality_score": 93.11,
            "available_pre_periods": 7,
            "available_post_periods": 16,
            "evidence_threshold_status": "passed",
            "period_requirement_status": "passed",
            "exclusion_reasons": ["county_first_entry_not_verified"],
        },
        "41051": {
            "facility_id": "fac_im3_building_00369452572",
            "source_id": "src_sec_pittock_block_2021",
            "when": {"year": 2003, "precision": "year"},
            "data_quality_score": 76.82,
            "available_pre_periods": 2,
            "available_post_periods": 21,
            "evidence_threshold_status": "failed",
            "period_requirement_status": "failed",
        ...
        "42003": {
            "facility_id": "fac_im3_building_00537807168",
            "source_id": "src_expedient_history_2003",
            "when": {"date": "2003-10-01", "precision": "month"},
            "data_quality_score": 89.38,
            "available_pre_periods": 2,
            "available_post_periods": 21,
            "evidence_threshold_status": "passed",
            "period_requirement_status": "failed",
        ...
        "42077": {
            "facility_id": "fac_im3_building_00296374269",
            "source_id": "src_flexential_pci_aoc_2022",
            "when": {"year": 2022, "precision": "year"},
        ...
        "46099": {
            "facility_id": "fac_im3_building_00239032142",
            "source_id": "src_tierpoint_cosentry_acquisition_20160331",
            "when": {"date": "2016-03-31", "precision": "day"},
            "data_quality_score": 95.06,
            "available_pre_periods": 15,
            "available_post_periods": 8,
            "evidence_threshold_status": "passed",
        ...
        "47157": {
            "facility_id": "fac_im3_building_01381264361",
            "source_id": "src_xai_series_c_20241223",
            "when": {"date": "2024-12-23", "precision": "day"},
            "data_quality_score": 96.04,
            "available_pre_periods": 23,
            "available_post_periods": 0,
            "evidence_threshold_status": "passed",
        ...
        "48121": {
            "facility_id": "fac_im3_building_00507673418",
            "source_id": "src_tdlr_citi_southwest_kiosk_20160809",
            "when": {"date": "2016-08-09", "precision": "day"},
            "data_quality_score": 92.12,
            "available_pre_periods": 15,
            "available_post_periods": 8,
            "evidence_threshold_status": "passed",
            "period_requirement_status": "passed",
        ...
        "53053": {
            "facility_id": "fac_im3_building_00634638463",
            "source_id": "src_puyallup_centeris_sh1_open_house_20160803",
            "when": {"date": "2016-08-03", "precision": "day"},
            "data_quality_score": 94.08,
            "available_pre_periods": 15,
            "available_post_periods": 8,
            "evidence_threshold_status": "passed",
        ...
        "41059": {
            "facility_id": "fac_im3_building_00592257327",
            "source_id": "src_or_federal_court_pdx51_pdx54_20210719",
            "when": {"date": "2021-07-19", "precision": "day"},
            "data_quality_score": 88.35,
            "available_pre_periods": 20,
            "available_post_periods": 3,
            "evidence_threshold_status": "passed",
            "period_requirement_status": "passed",
        ...
        "48029": {
            "facility_id": "fac_im3_building_00364289074",
            "source_id": "src_microsoft_san_antonio_energy_20131104",
            "when": {"date": "2013-11-04", "precision": "day"},
            "data_quality_score": 96.04,
            "available_pre_periods": 12,
            "available_post_periods": 11,
            "evidence_threshold_status": "passed",
        ...
        "08031": {
            "facility_id": "fac_im3_point_12112236372",
            "source_id": "src_flexential_soc3_locations_2023",
            "when": {"year": 2023, "precision": "year"},
            "data_quality_score": 76.82,
            "available_pre_periods": 22,
            "available_post_periods": 1,
        ...
        "13135": {
            "facility_id": "fac_im3_building_00387378765",
            "source_id": "src_qts_2020_10k_suwanee",
            "when": {"date": "2020-12-31", "precision": "day"},
            "data_quality_score": 97.02,
            "available_pre_periods": 19,
            "available_post_periods": 4,
            "evidence_threshold_status": "passed",
        ...
        "29510": {
            "facility_id": "fac_im3_building_00617511179",
            "source_id": "src_wustl_genome_data_center_maintenance_20250917",
            "when": {"date": "2025-09-17", "precision": "day"},
            "data_quality_score": 96.04,
            "available_pre_periods": 24,
            "available_post_periods": 0,
            "evidence_threshold_status": "passed",
        ...
        "34031": {
            "facility_id": "fac_im3_building_00460177088",
            "source_id": "src_digitalrealty_2015_10k_clifton",
            "when": {"year": 2015, "precision": "year"},
            "data_quality_score": 78.41,
            "available_pre_periods": 14,
            "available_post_periods": 9,
            "evidence_threshold_status": "failed",
        ...
        "36001": {
            "facility_id": "fac_im3_building_00871694597",
            "source_id": "src_timesunion_turnkey_latham_opening_20120416",
            "when": {"date": "2012-04-01", "precision": "month"},
            "data_quality_score": 80.37,
            "available_pre_periods": 11,
            "available_post_periods": 12,
            "evidence_threshold_status": "failed",
            "period_requirement_status": "passed",
        ...
        "36061": {
            "facility_id": "fac_im3_building_00289884123",
            "source_id": "src_nyc_lpc_33_thomas_completed_1974",
            "when": {"year": 1974, "precision": "year"},
            "data_quality_score": 74.48,
            "available_pre_periods": 0,
            "available_post_periods": 24,
            "evidence_threshold_status": "failed",
        ...
        "37161": {
            "facility_id": "fac_im3_building_00460089167",
            "source_id": "src_meta_forest_city_open_20120419",
            "when": {"date": "2012-04-19", "precision": "day"},
            "data_quality_score": 98.01,
            "available_pre_periods": 11,
            "available_post_periods": 12,
            "evidence_threshold_status": "passed",
        ...
        "47165": {
            "facility_id": "fac_im3_building_01294595615",
            "source_id": "src_meta_gallatin_online_20241114",
            "when": {"date": "2024-11-14", "precision": "day"},
            "data_quality_score": 98.01,
            "available_pre_periods": 23,
            "available_post_periods": 0,
            "evidence_threshold_status": "passed",
        ...
        "53063": {
            "facility_id": "fac_im3_building_00335468616",
            "source_id": "src_tierpoint_spokane_current_2026",
            "when": {"year": 2026, "precision": "year"},
            "data_quality_score": 76.05,
            "available_pre_periods": 24,
            "available_post_periods": 0,
            "evidence_threshold_status": "failed",
        ...
        "55079": {
            "facility_id": "fac_im3_building_00664938835",
            "source_id": "src_expedient_milwaukee_open_20211020",
            "when": {"date": "2021-10-20", "precision": "day"},
            "data_quality_score": 98.01,
            "available_pre_periods": 20,
            "available_post_periods": 3,
            "evidence_threshold_status": "passed",
        ...
        "13215": {
            "facility_id": "fac_im3_building_00958153770",
            "source_id": "src_peeringdb_dli_one_20250926",
            "when": {"date": "2025-09-26", "precision": "day"},
        ...
        "13097": {
            "facility_id": "fac_im3_building_01097502168",
            "source_id": "src_flexential_douglasville_digitalocean_20250318",
            "when": {"date": "2025-03-18", "precision": "day"},
            "data_quality_score": 98.01,
            "available_pre_periods": 24,
            "available_post_periods": 0,
            "evidence_threshold_status": "passed",
        ...
        "37183": {
            "facility_id": "fac_im3_point_12112361725",
            "source_id": "src_flexential_soc3_locations_2023",
            "when": {"date": "2023-10-31", "precision": "day"},
            "data_quality_score": 98.01,
            "available_pre_periods": 22,
            "available_post_periods": 1,
            "evidence_threshold_status": "passed",
        ...
        "39035": {
            "facility_id": "fac_im3_building_01382205749",
            "source_id": "src_t5_cleveland_onesheet_202008",
            "when": {"date": "2020-08-01", "precision": "month"},
            "data_quality_score": 91.24,
            "available_pre_periods": 19,
            "available_post_periods": 4,
            "evidence_threshold_status": "passed",
            "period_requirement_status": "passed",
        ...
        "47125": {
            "facility_id": "fac_im3_building_00844389014",
            "source_id": "src_google_clarksville_working_20190410",
            "when": {"date": "2019-04-10", "precision": "day"},
            "data_quality_score": 98.01,
            "available_pre_periods": 18,
            "available_post_periods": 5,
            "evidence_threshold_status": "passed",
        ...
        "51087": {
            "facility_id": "fac_im3_building_01132541700",
            "source_id": "src_meta_henrico_online_20200805",
            "when": {"date": "2020-08-05", "precision": "day"},
            "data_quality_score": 98.01,
            "available_pre_periods": 19,
            "available_post_periods": 4,
            "evidence_threshold_status": "passed",
        ...
        "29047": {
            "facility_id": "fac_im3_building_01184066765",
            "source_id": "src_meta_kansas_city_online_20250820",
            "when": {"date": "2025-08-20", "precision": "day"},
            "data_quality_score": 98.01,
            "available_pre_periods": 24,
            "available_post_periods": 0,
            "evidence_threshold_status": "passed",
        ...
        "01089": {
            "facility_id": "fac_im3_building_00408012357",
            "source_id": "src_asa_supercomputer_center_history_198710",
            "when": {"date": "1987-10-01", "precision": "month"},
            "data_quality_score": 93.11,
            "available_pre_periods": 0,
            "available_post_periods": 24,
            "evidence_threshold_status": "passed",
        ...
        "06001": {
            "facility_id": "fac_im3_building_00383728972",
            "source_id": "src_he_fremont_open_20010717",
            "when": {"date": "2001-07-17", "precision": "day"},
            "data_quality_score": 98.01,
            "available_pre_periods": 0,
            "available_post_periods": 23,
            "evidence_threshold_status": "passed",
            "period_requirement_status": "failed",
            "exclusion_reasons": ["panel_period_requirement_not_met", "county_first_entry_not_verified"],
        },
        "06067": {
            "facility_id": "fac_im3_building_00032560292",
            "source_id": "src_mas_ntt_ca1_valuation_2025",
            "when": {"year": 2001, "precision": "year"},
            "data_quality_score": 78.41,
            "available_pre_periods": 0,
            "available_post_periods": 23,
            "evidence_threshold_status": "failed",
        ...
        "19181": {
            "facility_id": "fac_im3_building_01318322071",
            "source_id": "src_wdm_development_retrospective_2023",
            "when": {"date": "2023-09-01", "precision": "month"},
            "data_quality_score": 90.31,
            "available_pre_periods": 22,
            "available_post_periods": 1,
            "evidence_threshold_status": "passed",
        ...
        "24510": {
            "facility_id": "fac_im3_building_00336387583",
            "source_id": "src_dck_baltimore_technology_park_open_20060308",
            "when": {"date": "2006-03-01", "precision": "month"},
            "data_quality_score": 83.94,
            "available_pre_periods": 5,
            "available_post_periods": 18,
            "evidence_threshold_status": "failed",
            "period_requirement_status": "failed",
        ...
        "35061": {
            "facility_id": "fac_im3_building_00626785488",
            "source_id": "src_meta_los_lunas_open_20190207",
            "when": {"date": "2019-02-07", "precision": "day"},
            "data_quality_score": 98.01,
            "available_pre_periods": 18,
            "available_post_periods": 5,
            "evidence_threshold_status": "passed",
        ...
        "47187": {
            "facility_id": "fac_im3_building_00942604537",
            "source_id": "src_flexential_soc3_locations_2023",
            "when": {"date": "2023-10-31", "precision": "day"},
            "data_quality_score": 98.01,
            "available_pre_periods": 22,
        ...
        "49035": {
            "facility_id": "fac_im3_building_00355926681",
            "source_id": "src_flexential_soc3_locations_2023",
            "when": {"date": "2023-10-31", "precision": "day"},
            "data_quality_score": 98.01,
            "available_pre_periods": 22,
        ...
        "53025": {
            "facility_id": "fac_im3_building_00499403180",
            "source_id": "src_microsoft_quincy_since_2007_202305",
            "when": {"year": 2007, "precision": "year"},
            "data_quality_score": 78.41,
            "available_pre_periods": 6,
            "available_post_periods": 17,
        ...
        "06073": {
            "facility_id": "fac_im3_building_00031800424",
            "source_id": "src_dck_ais_lightwave_open_20080919",
            "when": {"date": "2008-09-18", "precision": "day"},
            "data_quality_score": 88.36,
            "available_pre_periods": 7,
            "available_post_periods": 16,
            "evidence_threshold_status": "failed",
            "period_requirement_status": "passed",
        ...
        "27053": {
            "facility_id": "fac_im3_building_01343100173",
            "source_id": "src_databank_msp3_open_20211104",
            "when": {"date": "2021-11-04", "precision": "day"},
            "data_quality_score": 98.01,
            "available_pre_periods": 20,
            "available_post_periods": 3,
            "evidence_threshold_status": "passed",
        ...
        "40143": {
            "facility_id": "fac_im3_point_12793428582",
            "source_id": "src_tulsaconnect_soc1_dc3_20240430",
            "when": {"date": "2024-04-30", "precision": "day"},
            "data_quality_score": 98.01,
            "available_pre_periods": 23,
            "available_post_periods": 0,
        ...
        "08035": {
            "facility_id": "fac_im3_building_00377545385",
            "source_id": "src_flexential_soc3_locations_2023",
            "when": {"date": "2023-10-31", "precision": "day"},
            "data_quality_score": 98.01,
            "available_pre_periods": 22,
        ...
        "12095": {
            "facility_id": "fac_im3_building_00609126465",
            "source_id": "src_hostdime_orlando_existing_20210624",
            "when": {"date": "2021-06-24", "precision": "day"},
            "data_quality_score": 98.01,
            "available_pre_periods": 20,
            "available_post_periods": 3,
            "evidence_threshold_status": "passed",
        ...
        "29095": {
            "facility_id": "fac_im3_point_09922928085",
            "source_id": "src_netrality_1102_grand_upgrade_20200427",
            "when": {"date": "2020-04-27", "precision": "day"},
            "data_quality_score": 98.01,
            "available_pre_periods": 19,
            "available_post_periods": 4,
            "evidence_threshold_status": "passed",
        ...
        "36029": {
            "facility_id": "fac_im3_building_00916148870",
            "source_id": "src_uniland_crosspoint_existing_dc_2017",
            "when": {"year": 2017, "precision": "year"},
            "data_quality_score": 73.73,
            "available_pre_periods": 16,
            "available_post_periods": 7,
            "evidence_threshold_status": "failed",
            "period_requirement_status": "passed",
        ...
        "39061": {
            "facility_id": "fac_im3_building_00046128119",
            "source_id": "src_sec_cyrusone_7th_street_20130510",
            "when": {"date": "2013-05-10", "precision": "day"},
            "data_quality_score": 98.01,
            "available_pre_periods": 12,
            "available_post_periods": 11,
            "evidence_threshold_status": "passed",
        ...
        "48339": {
            "facility_id": "fac_im3_point_09190480200",
            "source_id": "src_stream_houston_i_since_2013",
            "when": {"year": 2013, "precision": "year"},
            "data_quality_score": 78.41,
            "available_pre_periods": 12,
            "available_post_periods": 11,
        ...
        "55025": {
            "facility_id": "fac_im3_point_13226718075",
            "source_id": "src_lumen_iso_madison_20250617",
            "when": {"date": "2025-06-17", "precision": "day"},
            "data_quality_score": 98.01,
            "available_pre_periods": 24,
            "available_post_periods": 0,
            "evidence_threshold_status": "passed",
        ...
        "06075": {
            "facility_id": "fac_im3_building_00289620443",
            "source_id": "src_sec_digital_200_paul_20120228",
            "when": {"date": "2012-02-28", "precision": "day"},
            "data_quality_score": 98.01,
            "available_pre_periods": 11,
            "available_post_periods": 12,
            "evidence_threshold_status": "passed",
        ...
        "08041": {
            "facility_id": "fac_im3_building_00813532303",
            "source_id": "src_sap_cloud_dc_list_202210",
            "when": {"date": "2022-10-01", "precision": "month"},
            "data_quality_score": 93.11,
            "available_pre_periods": 21,
            "available_post_periods": 2,
            "evidence_threshold_status": "passed",
        ...
        "12086": {
            "facility_id": "fac_im3_building_00435518280",
            "source_id": "src_equinix_mi1_history_20180820",
            "when": {"year": 2001, "precision": "year"},
            "data_quality_score": 78.41,
            "available_pre_periods": 0,
            "available_post_periods": 23,
        ...
        "48439": {
            "facility_id": "fac_im3_building_00500820007",
            "source_id": "src_meta_fort_worth_traffic_20171211",
            "when": {"date": "2017-12-11", "precision": "day"},
            "data_quality_score": 98.01,
            "available_pre_periods": 16,
            "available_post_periods": 7,
            "evidence_threshold_status": "passed",
        ...
        "19049": {
            "facility_id": "fac_im3_building_00438077632",
            "source_id": "src_iowa_dnr_alluvium_current_20260831",
            "when": {"date": "2026-08-31", "precision": "day"},
            "data_quality_score": 98.01,
            "available_pre_periods": 24,
            "available_post_periods": 0,
            "evidence_threshold_status": "passed",
        ...
        "39159": {
            "facility_id": "fac_im3_building_01252196805",
            "source_id": "src_baxtel_aws_industrial_parkway_2024",
            "when": {"year": 2024, "precision": "year"},
            "data_quality_score": 70.69,
            "available_pre_periods": 23,
            "available_post_periods": 0,
            "evidence_threshold_status": "failed",
        ...
        "45015": {
            "facility_id": "fac_im3_building_00610827836",
            "source_id": "src_google_berkeley_built_2007",
            "when": {"year": 2007, "precision": "year"},
            "data_quality_score": 78.41,
            "available_pre_periods": 6,
            "available_post_periods": 17,
        ...
        "49049": {
            "facility_id": "fac_im3_building_01024051963",
            "source_id": "src_meta_utah_online_20210714",
            "when": {"date": "2021-07-14", "precision": "day"},
            "data_quality_score": 98.01,
            "available_pre_periods": 20,
            "available_post_periods": 3,
            "evidence_threshold_status": "passed",
        ...
        "04003": {
            "facility_id": "fac_im3_building_01042260902",
            "source_id": "src_dcd_directv_arizona_20240725",
            "when": {"year": 2006, "precision": "year"},
            "data_quality_score": 70.69,
            "available_pre_periods": 5,
            "available_post_periods": 18,
        ...
        "04019": {
            "facility_id": "fac_im3_building_00843314844",
            "source_id": "src_dcd_directv_arizona_20240725",
            "when": {"year": 2007, "precision": "year"},
            "data_quality_score": 70.69,
            "available_pre_periods": 6,
        ...
        "24021": {
            "facility_id": "fac_im3_building_00178331621",
            "source_id": "src_federal_register_ics_frederick_20080605",
            "when": {"date": "2008-06-05", "precision": "day"},
            "data_quality_score": 97.02,
            "available_pre_periods": 7,
            "available_post_periods": 16,
            "evidence_threshold_status": "passed",
        ...
        "24027": {
            "facility_id": "fac_im3_building_01143804764",
            "source_id": "src_howard_fiscal_analysis_cb31_2026",
            "when": {"year": 2023, "precision": "year"},
            "data_quality_score": 70.66,
            "available_pre_periods": 22,
            "available_post_periods": 1,
            "evidence_threshold_status": "failed",
            "period_requirement_status": "failed",
        ...
        "25009": {
            "facility_id": "fac_im3_building_00211832626",
            "source_id": "src_maine_tierpoint_soc_bridge_20191031",
            "when": {"date": "2019-10-31", "precision": "day"},
            "data_quality_score": 94.08,
            "available_pre_periods": 18,
            "available_post_periods": 5,
            "evidence_threshold_status": "passed",
        ...
        "26049": {
            "facility_id": "fac_im3_building_00755540694",
            "source_id": "src_otava_flint_history_2013",
            "when": {"year": 2005, "precision": "year"},
            "data_quality_score": 72.13,
            "available_pre_periods": 4,
            "available_post_periods": 19,
            "evidence_threshold_status": "failed",
            "period_requirement_status": "failed",
        ...
        "33017": {
            "facility_id": "fac_im3_building_00079056848",
            "source_id": "src_ntia_usnh_progress_report_20130320",
            "when": {"date": "2012-12-31", "precision": "day"},
            "data_quality_score": 97.02,
            "available_pre_periods": 11,
            "available_post_periods": 12,
            "evidence_threshold_status": "passed",
        ...
        "39041": {
            "facility_id": "fac_im3_building_01319724559",
            "source_id": "src_sdoh_paulus_citi_operating_20130930",
            "when": {"date": "2008-08-01", "precision": "month"},
            "data_quality_score": 91.24,
            "available_pre_periods": 7,
            "available_post_periods": 16,
            "evidence_threshold_status": "passed",
        ...
        "40101": {
            "facility_id": "fac_im3_building_01387080629",
            "source_id": "src_southwest_ledger_yzy_operational_20240220",
            "when": {"date": "2024-02-20", "precision": "day"},
            "data_quality_score": 90.25,
            "available_pre_periods": 23,
            "available_post_periods": 0,
            "evidence_threshold_status": "failed",
        ...
        "41017": {
            "facility_id": "fac_im3_building_00752001022",
            "source_id": "src_dcd_cascade_divide_bend_20210730",
            "when": {"date": "2021-07-30", "precision": "day"},
            "data_quality_score": 90.24,
            "available_pre_periods": 20,
            "available_post_periods": 3,
            "evidence_threshold_status": "failed",
            "period_requirement_status": "passed",
        ...
        "55015": {
            "facility_id": "fac_im3_building_00766768556",
            "source_id": "src_ark_wisconsin_opening_20240613",
            "when": {"date": "2024-06-12", "precision": "day"},
            "data_quality_score": 97.02,
            "available_pre_periods": 23,
            "available_post_periods": 0,
            "evidence_threshold_status": "passed",
        ...
        "55101": {
            "facility_id": "fac_im3_building_00919948160",
            "source_id": "src_dcd_foxconn_globe_opening_20210519",
            "when": {"date": "2021-05-19", "precision": "day"},
            "data_quality_score": 88.36,
            "available_pre_periods": 20,
            "available_post_periods": 3,
            "evidence_threshold_status": "failed",
        ...
        "48139": {
            "facility_id": "fac_im3_building_00844386860",
            "source_id": "src_google_dallas_cloud_region_20220607",
            "when": {"date": "2022-06-07", "precision": "day"},
            "data_quality_score": 91.08,
            "available_pre_periods": 21,
            "available_post_periods": 2,
            "evidence_threshold_status": "passed",
            "period_requirement_status": "failed",
        ...
        "08005": {
            "facility_id": "fac_im3_building_00292506452",
            "source_id": "src_databank_den2_purchase_20220202",
            "when": {"date": "2022-02-02", "precision": "day"},
            "data_quality_score": 97.02,
            "available_pre_periods": 21,
            "available_post_periods": 2,
            "evidence_threshold_status": "passed",
        ...
        "51061": {
            "facility_id": "fac_im3_building_00225745958",
            "source_id": "src_ovhcloud_vint_hill_anniversary_20230517",
            "when": {"precision": "year", "year": 2018},
            "data_quality_score": 77.62,
            "available_pre_periods": 17,
            "available_post_periods": 6,
            "evidence_threshold_status": "failed",
        ...
        "48453": {
            "facility_id": "fac_im3_building_00379205427",
            "source_id": "src_datafoundry_history_current_2026",
            "when": {"precision": "year", "year": 2003},
            "data_quality_score": 76.05,
            "available_pre_periods": 2,
            "available_post_periods": 21,
            "evidence_threshold_status": "failed",
        ...
        "31055": {
            "facility_id": "fac_im3_building_00818167571",
            "source_id": "src_1623_farnam_expansion_20200616",
            "when": {"date": "2020-06-16", "precision": "day"},
            "data_quality_score": 96.04,
            "available_pre_periods": 19,
            "available_post_periods": 4,
            "evidence_threshold_status": "passed",
        ...
        "13217": {
            "facility_id": "fac_im3_building_01054387471",
            "source_id": "src_meta_stanton_operating_20260403",
            "when": {"date": "2026-04-03", "precision": "day"},
        ...
        "17037": {
            "facility_id": "fac_im3_building_01312792611",
            "source_id": "src_illinois_meta_dekalb_opening_20231129",
            "when": {"date": "2023-11-29", "precision": "day"},
            "data_quality_score": 98.01,
            "available_pre_periods": 22,
            "available_post_periods": 1,
            "evidence_threshold_status": "passed",
        ...
        "20091": {
            ...
            "source_id": "src_sec_state_farm_olathe_20250203",
            "when": {"date": "2016-11-03", "precision": "day"},
            "data_quality_score": 98.01,
            "available_pre_periods": 15,
            "available_post_periods": 8,
            "evidence_threshold_status": "passed",
        ...
        "19155": {
            ...
            "source_id": "src_google_iowa_datacenter_2009",
            "when": {"precision": "year", "year": 2009},
            "data_quality_score": 76.83,
            "available_pre_periods": 8,
            "available_post_periods": 15,
            "evidence_threshold_status": "failed",
        ...
        "41067": {
            "facility_id": "fac_im3_building_00328703599",
            "source_id": "src_flexential_viawest_oregon_20160927",
            "when": {"precision": "year", "year": 2006},
            "data_quality_score": 76.83,
            "available_pre_periods": 5,
            "available_post_periods": 18,
            "evidence_threshold_status": "failed",
        ...
        "36063": {
            "facility_id": "fac_im3_building_00832622369",
            "source_id": "src_sec_blockfusion_niagara_20251119",
            "when": {"date": "2025-11-19", "precision": "day"},
            "data_quality_score": 98.01,
            "available_pre_periods": 24,
            "available_post_periods": 0,
            "evidence_threshold_status": "passed",
        ...
        "41049": {
            "facility_id": "fac_im3_building_00460070144",
            "source_id": "src_amazon_eastern_oregon_20250826",
            "when": {"precision": "year", "year": 2011},
            "data_quality_score": 76.83,
            "available_pre_periods": 10,
            "available_post_periods": 13,
            "evidence_threshold_status": "failed",
        ...
        "26163": {
            "facility_id": "fac_im3_building_00903236619",
            "source_id": "src_rocket_quicken_technology_center_20150630",
            "when": {"date": "2015-06-30", "precision": "day"},
            "data_quality_score": 98.01,
            "available_pre_periods": 14,
            "available_post_periods": 9,
            "evidence_threshold_status": "passed",
        ...
        "32029": {
            "facility_id": "fac_im3_point_06685432442",
            "source_id": "src_switch_tahoe_reno_open_20170215",
            "when": {"date": "2017-02-15", "precision": "day"},
            "data_quality_score": 98.01,
            "available_pre_periods": 16,
            "available_post_periods": 7,
            "evidence_threshold_status": "passed",
        ...
        "29165": {
            "facility_id": "fac_im3_building_00195165374",
            "source_id": "src_kcmo_energy_benchmarking_2022",
            "when": {"precision": "year", "year": 2022},
            "data_quality_score": 73.73,
            "available_pre_periods": 21,
            "available_post_periods": 2,
            "evidence_threshold_status": "failed",
        ...
        "48201": {
            "facility_id": "fac_im3_building_00671838900",
            "source_id": "src_rice_datacenter_open_20070802",
            "when": {"date": "2007-08-02", "precision": "day"},
            "data_quality_score": 98.01,
            "available_pre_periods": 6,
            "available_post_periods": 17,
            "evidence_threshold_status": "passed",
    ...
    expected_research_initial_fips = [
        "13121", "34017", "32003", "18105", "26125", "17031", "12001", "23005",
        "25017", "04013", "12057", "06085", "42091", "37119", "13067", "33015",
    ...
    for evidence_path in CONFIG_DIR.glob("*evidence-sources.json"):
        for record in load_json(evidence_path).get("records", []):
            all_evidence_source_records[record.get("source_id", "")] = record
    ...
    resolution_status_counts = Counter(record.get("resolution_status") for record in resolution_candidates)
    ...
    successor_adjudication_by_candidate_id = {
        record.get("resolution_candidate_id"): record
    ...
    for index, record in enumerate(resolution_candidates):
        ...
        successor_adjudication = successor_adjudication_by_candidate_id.get(record.get("resolution_candidate_id"))
        ...
        successor_lineage_valid = (
            successor_adjudication is not None
            and record.get("resolution_status") == "evidence_collected"
            and record.get("resolution_adjudication_id") == successor_adjudication.get("resolution_adjudication_id")
            and record.get("latest_resolution_state") == successor_adjudication.get("resolution_state")
            and successor_adjudication.get("county_fips") == county_fips
            and successor_adjudication.get("county_name") == record.get("county_name")
            and successor_adjudication.get("state_abbr") == record.get("state_abbr")
            and successor_adjudication.get("candidate_generation") == record.get("candidate_generation")
            and successor_adjudication.get("reviewed_anchor") == record.get("candidate_anchor")
            and successor_adjudication.get("prior_adjudication_id") == record.get("prior_adjudication_id")
        ) if successor_adjudication is not None else (
            record.get("resolution_status") == "queued"
            and "resolution_adjudication_id" not in record
            and "latest_resolution_state" not in record
        )
        if (
            ...
            or not successor_lineage_valid
            or record.get("score_components") != expected_components
            or record.get("priority_score") != expected_score
    ...
    resolution_public_evidence_records = resolution_public_evidence.get("records", [])
    expected_resolution_source_ids = sorted({
        source_id for record in resolution_tranche_adjudications
        for source_id in record.get("source_ids", [])
    })
    if (
        resolution_public_evidence.get("record_count") != len(expected_resolution_source_ids)
        or [record.get("source_id") for record in resolution_public_evidence_records] != expected_resolution_source_ids
        or any(record != all_evidence_source_records.get(record.get("source_id")) for record in resolution_public_evidence_records)
    ):
        issues.append(Issue("public_data_validation", resolution_public_evidence_path.name, "public successor evidence-source projection is inconsistent"))
    for index, record in enumerate(resolution_public_evidence_records):
        for issue in validator.validate_record(record, schema_paths["source"]):
            issues.append(Issue("public_data_validation", f"{resolution_public_evidence_path.name}.records[{index}]{issue.path[1:]}", issue.message))
    ...
    if resolution_manifest.get("record_count") != resolution_manifest_total:
    ...
    if national_tranche_2_sources_document.get("record_count") != 14 or len(national_tranche_2_sources) != 14:
        issues.append(Issue("public_data_validation", national_tranche_2_sources_path.name, "expected fourteen governed evidence sources"))
    ...
    if national_tranche_3_sources_document.get("record_count") != 22 or len(national_tranche_3_sources) != 22:
        issues.append(Issue("public_data_validation", national_tranche_3_sources_path.name, "expected twenty-two governed evidence sources"))
    ...
    if national_tranche_4_sources_document.get("record_count") != 18 or len(national_tranche_4_sources) != 18:
        issues.append(Issue("public_data_validation", national_tranche_4_sources_path.name, "expected eighteen governed evidence sources"))
    ...
    if national_tranche_5_sources_document.get("record_count") != 17 or len(national_tranche_5_sources) != 17:
        issues.append(Issue("public_data_validation", national_tranche_5_sources_path.name, "expected seventeen governed evidence sources"))
