...
class CountyTreatmentAnchorReviewTest(unittest.TestCase):
    ...
    def test_adjudication_tranches_are_explicit_and_not_causal_ready(self):
        ...
        self.assertEqual(self.product["counts"]["adjudicated_counties"], 17)
        self.assertEqual(self.product["counts"]["causal_ready_counties"], 0)
        self.assertEqual(self.product["counts"]["unresolved_counties"], 18)
    ...
    def test_cherokee_linked_hosts_retain_temporal_gate_results(self):
        ...
        self.assertEqual(
            counties["19155"]["county_first_material_exposure_status"],
            "provisional_anchor_pending_first_exposure_confirmation",
        )
        self.assertEqual(counties["19155"]["adjudicated_date"], "2007-12-31")
        self.assertEqual(counties["19155"]["anticipation_date"], "2007-06-19")
        self.assertEqual(
            counties["20091"]["county_first_material_exposure_status"],
            "provisional_anchor_pending_first_exposure_confirmation",
        )
        self.assertEqual(counties["20091"]["adjudicated_date"], "2014-12-31")
        self.assertEqual(counties["20091"]["anticipation_date"], "2013-11-26")
        self.assertEqual(
            counties["37035"]["county_first_material_exposure_status"],
            "provisional_anchor_pending_first_exposure_confirmation",
        )
        self.assertEqual(counties["37035"]["adjudicated_date"], "2009-08-14")
        self.assertEqual(counties["37035"]["anticipation_date"], "2009-07-06")

    def test_eleven_counties_preserve_full_seven_domain_audits(self):
        counties = {county["county_fips"]: county for county in self.product["counties"]}
        expected_codes = [
        ...
        for county_fips in ("01071", "18089", "19153", "19155", "20091", "32029", "35061", "37035", "47125", "48339", "49049"):
