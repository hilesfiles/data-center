...
class RejectedProjectStudyTest(unittest.TestCase):
    ...
    def test_build_preserves_distinct_proposals_and_sites(self):
        ...
        self.assertEqual(index["counts"]["projects"], 7)
        self.assertEqual(len(details), len(projects))
        self.assertEqual(len(details), len(sites))
        self.assertEqual(len({row["project_id"] for row in details}), 7)
        self.assertEqual(len({row["site_id"] for row in details}), 7)
        self.assertTrue(all(project["target_refs"][0]["entity_type"] == "proposed_site" for project in projects))
        self.assertTrue(all(detail["proposal_sector"] == "data_center" for detail in details))
        self.assertNotIn("prj_rejected_cascade_locks_nestle", {detail["project_id"] for detail in details})
    ...
    def test_enrichment_has_depth_and_primary_records(self):
        ...
        self.assertGreaterEqual(index["counts"]["sources"], 65)
        self.assertGreaterEqual(index["counts"]["primary_sources"], 30)
        self.assertGreaterEqual(index["counts"]["timeline_events"], 45)
    ...
    def test_county_outcome_windows_are_explicit(self):
        ...
        self.assertEqual(index["counts"]["counties_with_full_post_years"], 0)
        self.assertNotIn("41027", contexts)
