  ...
  return <>
    <section className="study-hero">
      ...
      <div className="study-counts" aria-label="Study coverage">
        ...
        <p>Initial research queue · expandable coverage<br />{study.counts.projects_with_economic_evidence} projects with economic evidence · {study.counts.economic_records} records</p>
      </div>
    </section>
...
function ProjectCard({ project: p }: { project: StudyProjectSummary }) {
  return <article className="project-card">
    ...
    <p className={`card-economics ${p.economic_record_count ? "has-evidence" : ""}`}>{p.economic_record_count ? `${p.economic_record_count} economic records · partial coverage` : "Economic evidence not yet collected"}</p>
...
function ProjectProfile({ summary, release, generatedAt }: { summary: StudyProjectSummary; release: string; generatedAt: string }) {
  ...
  return <article className="project-profile">
    ...
    {error ? <div className="error-panel" role="alert">{error}</div> : !detail ? <p role="status">Loading project evidence…</p> : <>
      ...
      {detail.economic_records.length > 0 && <EconomicAccounts key={detail.project_id} project={detail} />}
      <section className="project-section" aria-labelledby="gaps-title"><div className="section-heading"><h3 id="gaps-title">Evidence needed for the annual accounts</h3></div><p className="study-intro">{detail.economic_records.length ? "Available records provide a starting point. Partial coverage includes unresolved periods, project allocation or definitions; forecasts alone do not establish realized activity." : "Economic records have not yet been collected for this project."} Each category below identifies the evidence still needed.</p><div className="evidence-grid">{detail.evidence_gaps.map(g => <article key={g.code}><h4>{g.label}</h4><span className={g.status === "not_yet_collected" ? "missing-badge" : "coverage-badge"}>{{ not_yet_collected: "Not yet collected", partial: "Partial evidence", projections_only: "Projections only" }[g.status]}</span><p>{g.needed}</p></article>)}</div></section>
      <section className="project-section"><h3>Analysis readiness</h3><div className="readiness-list">{Object.keys(detail.analysis_readiness).map(key => <div key={key}><span>{{ construction: "Construction contribution", operations: "Operating employment", fiscal: "Local fiscal balance", causal: "Attributable economic effects" }[key]}</span><strong>Not assessed</strong></div>)}</div><p className="study-muted">Each analysis has its own evidence requirements. A county-first-entry decision does not determine whether this project's spending, jobs, or tax receipts can be documented.</p></section>
...
function Methodology({ study }: { study: StudyIndex }) {
  return <article className="study-methodology"><span className="eyebrow">Evidence & methodology</span><h2>Follow the project.<br />Measure the community.</h2><p className="methodology-lead">The study follows private-sector data-center construction, operation and expansion, linking documented contributions to community outcomes over time.</p>
    ...
    <section><h3>Separate questions, separate evidence</h3><ul><li><strong>Documented contributions:</strong> actual project investment, jobs, payroll, local purchasing and public receipts.</li><li><strong>Community trajectories:</strong> observed county employment, wages, output and population, with their own data vintages.</li><li><strong>Attributable effects:</strong> estimates requiring eligible events, comparison communities and model diagnostics.</li></ul><p>This release contains {study.counts.economic_records} source-checked economic records linked to {study.counts.projects_with_economic_evidence} candidates: {study.counts.reported_actual_records} reported activity records and {study.counts.projection_records} source projections. These are selected source claims, not harmonized project observations or a representative sample. A complete county history is context and does not itself establish a project's impact.</p></section>
    ...
    <section><h3>Release and reproducibility</h3><p>Candidate screen: {study.screen_date}. Published register: {study.release_id}.</p><p>Profiles retain source wording and date uncertainty. Economic sources and claims have their own version, page references, scope and review notes. The collected evidence leaves project allocation and complete annual harmonization unresolved; analysis readiness is not automatically advanced by adding source records.</p><a href={`${base}manifest.json`}>Publication manifest and source hashes ↗</a></section>
  </article>;
}
