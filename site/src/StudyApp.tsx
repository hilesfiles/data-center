  ...
  return <>
    <section className="study-hero">
      <div><span className="eyebrow">Private-sector development · United States</span>
        ...
        <a className="study-map-link" href="#/portfolio">See the 36-project findings <span aria-hidden="true">↗</span></a>
      </div>
...
function Methodology({ study }: { study: StudyIndex }) {
  return <article className="study-methodology"><span className="eyebrow">Evidence & methodology</span><h2>Follow the project.<br />Measure the community.</h2><p className="methodology-lead">The study follows private-sector data-center construction, operation and expansion, linking documented contributions to community outcomes over time.</p>
    ...
    <section><h3>The portfolio synthesis stops at Level 0</h3><p>The current cross-project analysis is limited to the 36 selected projects and their 35 host counties. It publishes evidence coverage, explicit gaps, and same-year reported distributions only where at least three projects and three counties share the same metric, unit, measure type, period kind, scope level, and allocation status.</p><p>It creates no new modeled value and no portfolio benefit total. Existing modeled syntheses remain separately labeled, and national comparison screening, pooled associations, and causal analysis are outside the current scope.</p><a href="#/portfolio">View the portfolio findings →</a></section>
...
export default function StudyApp() {
  ...
  useEffect(() => { document.title = `${hash === "#/portfolio" ? "Portfolio findings" : selected?.name ?? "Data Center Community Impact Observatory"} · Economic study`; }, [hash, selected]);
  if (hash === "#/map" || /^#\/county\/\d{5}$/.test(hash)) return <App study={study} studyError={error} />;
  const known = !hash || hash === "#/study" || hash === "#/portfolio" || hash === "#/methodology" || selected;
